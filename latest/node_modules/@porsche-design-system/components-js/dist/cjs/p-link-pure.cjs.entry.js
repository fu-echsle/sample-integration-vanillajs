...
const LinkPure = class {
    ...
    componentDidLoad() {
      ...
      outline-offset: 1px;
        ...
        index$1.transitionListener(this.linkTag, 'font-size', () => {
            const size = index$1.calcLineHeightForElement(this.linkTag);
    ...
    render() {
        ...
        const linkPureClasses = index$1.cx(index$1.prefix('link-pure'), index$1.mapBreakpointPropToPrefixedClasses('link-pure--size', this.size), index$1.prefix(`link-pure--theme-${this.theme}`), this.active && index$1.prefix('link-pure--active'));
        const iconClasses = index$1.cx(index$1.prefix('link-pure__icon'));
        const labelClasses = index$1.cx(index$1.prefix('link-pure__label'), index$1.mapBreakpointPropToPrefixedClasses('link-pure__label-', this.hideLabel, ['hidden', 'visible']));
        return (index.h(TagType, Object.assign({ class: linkPureClasses }, (TagType === 'a' ? {
            href: this.href,
            target: this.target,
            download: this.download,
            rel: this.rel
        } : null), { ref: el => this.linkTag = el }), index.h("p-icon", { class: iconClasses, color: 'inherit', size: 'inherit', name: this.icon, source: this.iconSource, ref: el => this.iconTag = el, "aria-hidden": 'true' }), index.h("p-text", { class: labelClasses, tag: 'span', color: 'inherit', size: 'inherit', weight: this.weight }, index.h("slot", null))));
    }
