import { r as registerInstance, f as createEvent, m as mapBreakpointPropToPrefixedClasses, c as prefix, h, g as getElement } from './breakpointCustomizable-063d6106.js';
import { t as throttle } from './index.esm-37b6c9b0.js';
import { c as classnames } from './index-45be8764.js';
import { i as improveFocusHandlingForCustomElement } from './focusHandling-e078221d.js';
...
class Pagination {
    ...
    render() {
        ...
        const paginationClasses = classnames(breakpointClasses, prefix('pagination'), prefix(`pagination--theme-${this.theme}`));
        const paginationItemsClasses = classnames(prefix('pagination__items'));
        ...
        const createPaginationItems = () => {
            ...
            let prevItem;
            let nextItem;
            const paginationItemClasses = classnames(prefix('pagination__item'));
            paginationModel.forEach((pageModel) => {
                if (pageModel.type === itemTypes.PREVIOUS_PAGE_LINK) {
                    const paginationPrevClasses = classnames(prefix('pagination__prev'), !pageModel.isActive && prefix('pagination__prev--disabled'));
                    return (prevItem = (h("li", Object.assign({}, pageModel, { class: paginationItemClasses }), h("span", { class: paginationPrevClasses, role: 'button', tabIndex: pageModel.isActive ? 0 : null, onClick: () => this.onClick(pageModel.value), onKeyDown: (e) => this.onKeyDown(e, pageModel.value), "aria-disabled": !pageModel.isActive && 'true', "aria-label": this.allyLabelPrev }, h("p-icon", { name: "arrow-head-left", color: "inherit" })))));
                }
                if (pageModel.type === itemTypes.ELLIPSIS) {
                    const paginationGoToClasses = classnames(prefix('pagination__goto'), prefix('pagination__goto--ellipsis'));
                    pageItems.push(h("li", Object.assign({}, pageModel, { class: paginationItemClasses }), h("span", { class: paginationGoToClasses })));
                }
                if (pageModel.type === itemTypes.PAGE) {
                    const paginationGoToClasses = classnames(prefix('pagination__goto'), pageModel.isActive && prefix('pagination__goto--current'));
                    pageItems.push(h("li", Object.assign({}, pageModel, { class: paginationItemClasses }), h("span", { class: paginationGoToClasses, role: 'button', tabIndex: pageModel.isActive ? null : 0, "aria-disabled": pageModel.isActive && 'true', onClick: () => this.onClick(pageModel.value), onKeyDown: (e) => this.onKeyDown(e, pageModel.value), "aria-label": `${this.allyLabelPage} ${pageModel.value}`, "aria-current": pageModel.isActive && 'page' }, pageModel.value)));
                }
                if (pageModel.type === itemTypes.NEXT_PAGE_LINK) {
                    const paginationNextClasses = classnames(prefix('pagination__next'), !pageModel.isActive && prefix('pagination__next--disabled'));
                    return (nextItem = (h("li", Object.assign({}, pageModel, { class: paginationItemClasses }), h("span", { class: paginationNextClasses, role: 'button', tabIndex: pageModel.isActive ? 0 : null, onClick: () => this.onClick(pageModel.value), onKeyDown: (e) => this.onKeyDown(e, pageModel.value), "aria-disabled": !pageModel.isActive && 'true', "aria-label": this.allyLabelNext }, h("p-icon", { name: "arrow-head-right", color: "inherit" })))));
                }
            });
            return {
                prevItem,
        ...
        return (h("nav", { class: paginationClasses, role: "navigation", "aria-label": this.allyLabel, ref: (el) => (this.navigationElement = el) }, h("ul", { class: paginationItemsClasses }, paginationItems.prevItem, paginationItems.pageItems, paginationItems.nextItem)));
    }
