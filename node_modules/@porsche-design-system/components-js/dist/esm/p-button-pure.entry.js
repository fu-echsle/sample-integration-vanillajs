...
const ButtonPure = class {
    ...
    render() {
        const buttonPureClasses = cx(prefix('button-pure'), mapBreakpointPropToPrefixedClasses('button-pure--size', this.size), prefix(`button-pure--theme-${this.theme}`));
        const iconClasses = cx(prefix('button-pure__icon'));
        const labelClasses = cx(prefix('button-pure__label'), mapBreakpointPropToPrefixedClasses('button-pure__label-', this.hideLabel, ['hidden', 'visible']));
        const sublineClasses = cx(prefix('button-pure__subline'), mapBreakpointPropToPrefixedClasses('button-pure__subline-', this.hideLabel, ['hidden', 'visible']));
        return (h(Host, null, h("button", { class: buttonPureClasses, type: this.type, disabled: this.isDisabled(), tabindex: this.tabbable ? 0 : -1, ref: (el) => (this.buttonTag = el), "aria-busy": this.loading && 'true' }, this.loading ? (h("p-spinner", { class: iconClasses, size: "inherit", theme: this.theme, ref: (el) => (this.iconTag = el) })) : (h("p-icon", { class: iconClasses, color: "inherit", size: "inherit", name: this.icon, source: this.iconSource, ref: (el) => (this.iconTag = el), "aria-hidden": "true" })), h("p-text", { class: labelClasses, tag: "span", color: "inherit", size: "inherit", weight: this.weight }, h("slot", null))), this.subline && (h("p-text", { class: sublineClasses, color: "inherit", size: "inherit", tag: "div" }, h("slot", { name: "subline" })))));
    }
    setSubline() {
