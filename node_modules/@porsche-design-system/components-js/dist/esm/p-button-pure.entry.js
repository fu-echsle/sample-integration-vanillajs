...
import { c as calcLineHeightForElement } from './line-height-59b7344d.js';
...
class ButtonPure {
    ...
    render() {
        const buttonPureClasses = Object.assign({ [prefix('button-pure')]: true, [prefix(`button-pure--theme-${this.theme}`)]: true }, mapBreakpointPropToPrefixedClasses('button-pure--size', this.size));
        const iconClasses = prefix('button-pure__icon');
        const labelClasses = Object.assign({ [prefix('button-pure__label')]: true }, mapBreakpointPropToPrefixedClasses('button-pure__label-', this.hideLabel, ['hidden', 'visible']));
        const sublineClasses = Object.assign({ [prefix('button-pure__subline')]: true }, mapBreakpointPropToPrefixedClasses('button-pure__subline-', this.hideLabel, ['hidden', 'visible']));
        return (h(Host, null, h("button", { class: buttonPureClasses, type: this.type, disabled: this.isDisabled(), tabindex: this.tabbable ? 0 : -1, ref: (el) => (this.buttonTag = el), "aria-busy": this.loading && 'true' }, this.loading ? (h("p-spinner", { class: iconClasses, size: "inherit", theme: this.theme, ref: (el) => (this.iconTag = el) })) : (h("p-icon", { class: iconClasses, color: "inherit", size: "inherit", name: this.icon, source: this.iconSource, ref: (el) => (this.iconTag = el), "aria-hidden": "true" })), h("p-text", { class: labelClasses, tag: "span", color: "inherit", size: "inherit", weight: this.weight }, h("slot", null))), this.hasSubline && (h("p-text", { class: sublineClasses, color: "inherit", size: "inherit", tag: "div" }, h("slot", { name: "subline" })))));
    }
    checkHasSubline() {
