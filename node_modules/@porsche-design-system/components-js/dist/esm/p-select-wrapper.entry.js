...
const SelectWrapper = class {
    ...
    render() {
        ...
        const fakeOptionListClasses = cx(prefix('select-wrapper__fake-option-list'), { [prefix('select-wrapper__fake-option-list--hidden')]: this.fakeOptionListHidden });
        ...
        const messageClasses = cx(prefix('select-wrapper__message'), { [prefix(`select-wrapper__message--${this.state}`)]: this.state !== 'none' });
        return (h(Host, null, h("div", { class: labelClasses }, h("label", null, this.isLabelVisible &&
            h("p-text", { class: labelTextClasses, tag: 'span', color: 'inherit', onClick: () => this.labelClick() }, this.label || h("span", null, h("slot", { name: 'label' }))), this.isDescriptionVisible &&
            h("p-text", { class: descriptionTextClasses, tag: 'span', color: 'inherit', size: 'x-small', onClick: () => this.labelClick() }, this.description || h("span", null, h("slot", { name: 'description' }))), h("span", { class: fakeSelectClasses }, h("p-icon", { class: iconClasses, name: 'arrow-head-down', color: 'inherit' }), h("slot", null))), !this.isTouch &&
            h("div", { class: fakeOptionListClasses, role: 'listbox', "aria-activedescendant": `option-${this.optionSelected}`, tabIndex: -1, "aria-expanded": this.fakeOptionListHidden ? 'false' : 'true', "aria-labelledby": this.label, ref: el => this.fakeOptionListNode = el }, this.createFakeOptionList())), this.isMessageVisible &&
    ...
    createFakeOptionList() {
        return Array.from(this.options).map((option, key) => [
            ...
            h("div", { id: `option-${key}`, role: 'option', color: 'inherit', class: cx(prefix('select-wrapper__fake-option'), {
                ...
                }), onClick: () => !this.optionDisabled.includes(key) ? this.setOptionSelected(key) : this.select.focus(), "aria-selected": this.optionSelected === key && 'true', "aria-disabled": this.optionDisabled.includes(key) && 'true' }, h("span", null, option.text), key === this.optionSelected &&
