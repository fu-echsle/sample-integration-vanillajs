...
class SelectWrapper {
    ...
    render() {
        const labelClasses = classnames(prefix('select-wrapper__label'));
        const labelTextClasses = classnames(prefix('select-wrapper__label-text'), mapBreakpointPropToPrefixedClasses('select-wrapper__label-text-', this.hideLabel, ['hidden', 'visible']), { [prefix('select-wrapper__label-text--disabled')]: this.disabled });
        const descriptionTextClasses = classnames(prefix('select-wrapper__description-text'), mapBreakpointPropToPrefixedClasses('select-wrapper__description-text-', this.hideLabel, ['hidden', 'visible']), { [prefix('select-wrapper__description-text--disabled')]: this.disabled });
        const fakeSelectClasses = classnames(prefix('select-wrapper__fake-select'), { [prefix('select-wrapper__fake-select--disabled')]: this.disabled }, { [prefix(`select-wrapper__fake-select--${this.state}`)]: this.state !== 'none' });
        const fakeOptionListClasses = classnames(prefix('select-wrapper__fake-option-list'), {
            [prefix('select-wrapper__fake-option-list--hidden')]: this.fakeOptionListHidden
        });
        const iconClasses = classnames(prefix('select-wrapper__icon'), { [prefix('select-wrapper__icon--disabled')]: this.disabled }, { [prefix('select-wrapper__icon--opened')]: !this.fakeOptionListHidden });
        const messageClasses = classnames(prefix('select-wrapper__message'), {
            [prefix(`select-wrapper__message--${this.state}`)]: this.state !== 'none'
        });
        return (h(Host, null, h("div", { class: labelClasses }, h("label", null, this.isLabelVisible && (h("p-text", { class: labelTextClasses, tag: "span", color: "inherit", onClick: this.labelClick }, this.label || (h("span", null, h("slot", { name: "label" }))))), this.isDescriptionVisible && (h("p-text", { class: descriptionTextClasses, tag: "span", color: "inherit", size: "x-small", onClick: this.labelClick }, this.description || (h("span", null, h("slot", { name: "description" }))))), h("span", { class: fakeSelectClasses }, h("p-icon", { class: iconClasses, name: "arrow-head-down", color: "inherit" }), h("slot", null))), !this.isTouch && (h("div", { class: fakeOptionListClasses, role: "listbox", "aria-activedescendant": `option-${this.optionSelected}`, tabIndex: -1, "aria-expanded": this.fakeOptionListHidden ? 'false' : 'true', "aria-labelledby": this.label, ref: (el) => (this.fakeOptionListNode = el) }, this.createFakeOptionList()))), this.isMessageVisible && (h("p-text", { class: messageClasses, color: "inherit", role: this.state === 'error' && 'alert' }, this.message || (h("span", null, h("slot", { name: "message" })))))));
    }
    get isLabelVisible() {
    ...
    createFakeOptionList() {
        return Array.from(this.options).map((option, key) => [
            this.optgroups.length > 0 && option === option.parentNode.firstChild && (h("span", { class: classnames(prefix('select-wrapper__fake-optgroup-label')), role: "presentation" }, option.closest('optgroup').label)),
            h("div", { id: `option-${key}`, role: "option", color: "inherit", class: classnames(prefix('select-wrapper__fake-option'), {
                ...
                }), onClick: () => (!this.optionDisabled.includes(key) ? this.setOptionSelected(key) : this.select.focus()), "aria-selected": this.optionSelected === key && 'true', "aria-disabled": this.optionDisabled.includes(key) && 'true' }, h("span", null, option.text), key === this.optionSelected && (h("p-icon", { class: classnames(prefix('select-wrapper__fake-option-icon')), "aria-hidden": "true", name: "check", color: "inherit" })))
        ]);
    }
