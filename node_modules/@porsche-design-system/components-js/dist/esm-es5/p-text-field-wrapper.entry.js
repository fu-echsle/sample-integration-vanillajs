...
import { h as handleButtonEvent } from './buttonHandling-8f4ca036.js';
...
var TextFieldWrapper = /** @class */ (function () {
    function TextFieldWrapper(hostRef) {
        ...
        this.onSubmitHandler = function (event) {
            if (_this.isInputTypeSearch) {
                handleButtonEvent(event, _this.host, function () { return _this.searchButtonType; }, function () { return _this.disabled; });
            }
        };
    }
    ...
    TextFieldWrapper.prototype.render = function () {
        var _a, _b, _c, _d;
        var containerClasses = prefix('text-field-wrapper__container');
        var labelClasses = prefix('text-field-wrapper__label');
        var labelTextClasses = Object.assign((_a = {}, _a[prefix('text-field-wrapper__label-text')] = true, _a[prefix('text-field-wrapper__label-text--disabled')] = this.disabled, _a), mapBreakpointPropToPrefixedClasses('text-field-wrapper__label-text-', this.hideLabel, ['hidden', 'visible']));
        var descriptionTextClasses = Object.assign((_b = {}, _b[prefix('text-field-wrapper__description-text')] = true, _b[prefix('text-field-wrapper__description-text--disabled')] = this.disabled, _b), mapBreakpointPropToPrefixedClasses('text-field-wrapper__description-text-', this.hideLabel, [
            'hidden',
            'visible'
        ]));
        var fakeInputClasses = (_c = {},
            _c[prefix('text-field-wrapper__fake-input')] = true,
            _c[prefix("text-field-wrapper__fake-input--" + this.state)] = this.state !== 'none',
            _c[prefix('text-field-wrapper__fake-input--disabled')] = this.disabled,
            _c[prefix('text-field-wrapper__fake-input--readonly')] = this.readonly,
            _c);
        var buttonClasses = prefix('text-field-wrapper__button');
        var messageClasses = (_d = {},
            _d[prefix('text-field-wrapper__message')] = true,
            _d[prefix("text-field-wrapper__message--" + this.state)] = this.state !== 'none',
            _d);
        return (h(Host, null, h("div", { class: containerClasses }, h("label", { class: labelClasses }, this.isLabelVisible && (h("p-text", { class: labelTextClasses, tag: "span", color: "inherit", onClick: this.labelClick }, this.label || (h("span", null, h("slot", { name: "label" }))))), this.isDescriptionVisible && (h("p-text", { class: descriptionTextClasses, tag: "span", color: "inherit", size: "x-small", onClick: this.labelClick }, this.description || (h("span", null, h("slot", { name: "description" }))))), h("span", { class: fakeInputClasses }, h("slot", null))), this.isPasswordToggleable && (h("button", { type: "button", class: buttonClasses, onClick: this.togglePassword, disabled: this.disabled }, h("p-icon", { name: this.showPassword ? 'view-off' : 'view', color: "inherit" }))), this.isInputTypeSearch && (h("button", { onClick: this.onSubmitHandler, type: "submit", class: buttonClasses, disabled: this.disabled || this.readonly }, h("p-icon", { name: "search", color: "inherit" })))), this.isMessageVisible && (h("p-text", { class: messageClasses, color: "inherit", role: this.state === 'error' && 'alert' }, this.message || (h("span", null, h("slot", { name: "message" })))))));
    };
