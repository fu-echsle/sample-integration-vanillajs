...
import { r as registerInstance, n as isTouchDevice, d as prefix, m as mapBreakpointPropToPrefixedClasses, h, H as Host, e as transitionListener, j as insertSlottedStyles, g as getElement } from './index-e8247a5d.js';
...
var SelectWrapper = /** @class */ (function () {
    ...
    SelectWrapper.prototype.render = function () {
        ...
        var _this = this;
        var labelClasses = cx(prefix('select-wrapper__label'));
        var labelTextClasses = cx(prefix('select-wrapper__label-text'), mapBreakpointPropToPrefixedClasses('select-wrapper__label-text-', this.hideLabel, ['hidden', 'visible']), (_a = {}, _a[prefix('select-wrapper__label-text--disabled')] = this.disabled, _a));
        var descriptionTextClasses = cx(prefix('select-wrapper__description-text'), mapBreakpointPropToPrefixedClasses('select-wrapper__description-text-', this.hideLabel, ['hidden', 'visible']), (_b = {}, _b[prefix('select-wrapper__description-text--disabled')] = this.disabled, _b));
        var fakeSelectClasses = cx(prefix('select-wrapper__fake-select'), (_c = {}, _c[prefix('select-wrapper__fake-select--disabled')] = this.disabled, _c), (_d = {}, _d[prefix("select-wrapper__fake-select--" + this.state)] = this.state !== 'none', _d));
        var fakeOptionListClasses = cx(prefix('select-wrapper__fake-option-list'), (_e = {}, _e[prefix('select-wrapper__fake-option-list--hidden')] = this.fakeOptionListHidden, _e));
        var iconClasses = cx(prefix('select-wrapper__icon'), (_f = {}, _f[prefix('select-wrapper__icon--disabled')] = this.disabled, _f), (_g = {}, _g[prefix('select-wrapper__icon--opened')] = !this.fakeOptionListHidden, _g));
        var messageClasses = cx(prefix('select-wrapper__message'), (_h = {}, _h[prefix("select-wrapper__message--" + this.state)] = this.state !== 'none', _h));
        return (h(Host, null, h("div", { class: labelClasses }, h("label", null, this.isLabelVisible &&
            h("p-text", { class: labelTextClasses, tag: 'span', color: 'inherit', onClick: function () { return _this.labelClick(); } }, this.label || h("span", null, h("slot", { name: 'label' }))), this.isDescriptionVisible &&
            h("p-text", { class: descriptionTextClasses, tag: 'span', color: 'inherit', size: 'x-small', onClick: function () { return _this.labelClick(); } }, this.description || h("span", null, h("slot", { name: 'description' }))), h("span", { class: fakeSelectClasses }, h("p-icon", { class: iconClasses, name: 'arrow-head-down', color: 'inherit' }), h("slot", null))), !this.isTouch &&
            h("div", { class: fakeOptionListClasses, role: 'listbox', "aria-activedescendant": "option-" + this.optionSelected, tabIndex: -1, "aria-expanded": this.fakeOptionListHidden ? 'false' : 'true', "aria-labelledby": this.label, ref: function (el) { return _this.fakeOptionListNode = el; } }, this.createFakeOptionList())), this.isMessageVisible &&
            h("p-text", { class: messageClasses, color: 'inherit', role: this.state === 'error' && 'alert' }, this.message || h("span", null, h("slot", { name: 'message' })))));
    };
    ...
    SelectWrapper.prototype.bindStateListener = function () {
        var _this = this;
        transitionListener(this.select, 'border-top-color', function () {
            _this.setState();
        });
    };
    /*
    ...
    SelectWrapper.prototype.observeSelect = function () {
        ...
        this.selectObserver = new MutationObserver(function (mutations) {
            mutations.filter(function (mutation) { return mutation.type === 'childList'; }).forEach(function () {
                _this.setOptionList();
            });
            mutations.filter(function (mutation) { return mutation.type === 'attributes'; }).forEach(function () {
    ...
    SelectWrapper.prototype.setOptionList = function () {
    ...
    SelectWrapper.prototype.setOptionsDisabled = function () {
        var _this = this;
    ...
    SelectWrapper.prototype.setOptionSelected = function (key) {
    ...
    SelectWrapper.prototype.createFakeOptionList = function () {
        ...
        return Array.from(this.options).map(function (option, key) {
            var _a;
            return [
                (_this.optgroups.length > 0 && option === option.parentNode.firstChild) &&
                    h("span", { class: cx(prefix('select-wrapper__fake-optgroup-label')), role: 'presentation' }, option.closest('optgroup').label),
                h("div", { id: "option-" + key, role: 'option', color: 'inherit', class: cx(prefix('select-wrapper__fake-option'), (_a = {},
                        _a[prefix('select-wrapper__fake-option--selected')] = _this.optionSelected === key,
                        _a[prefix('select-wrapper__fake-option--highlighted')] = _this.optionHighlighted === key,
                        _a[prefix('select-wrapper__fake-option--disabled')] = _this.optionDisabled.includes(key),
                        _a)), onClick: function () { return !_this.optionDisabled.includes(key) ? _this.setOptionSelected(key) : _this.select.focus(); }, "aria-selected": _this.optionSelected === key && 'true', "aria-disabled": _this.optionDisabled.includes(key) && 'true' }, h("span", null, option.text), key === _this.optionSelected &&
                    h("p-icon", { class: cx(prefix('select-wrapper__fake-option-icon')), "aria-hidden": 'true', name: 'check', color: 'inherit' }))
            ];
        });
    };
...
export { SelectWrapper as p_select_wrapper };
