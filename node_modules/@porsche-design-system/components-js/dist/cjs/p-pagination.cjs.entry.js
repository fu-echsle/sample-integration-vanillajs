...
const handleResize = index.throttle(500, () => {
...
const Pagination = class {
    ...
    render() {
        const breakpointClasses = index.mapBreakpointPropToPrefixedClasses('pagination--size', this.maxNumberOfPageLinks);
        const pageRange = this.breakpointMaxNumberOfPageLinks === 7 ? 1 : 0;
        const paginationClasses = index$1.cx(breakpointClasses, index.prefix('pagination'), index.prefix(`pagination--theme-${this.theme}`));
        const paginationItemsClasses = index$1.cx(index.prefix('pagination__items'));
        ...
        const createPaginationItems = () => {
            ...
            let prevItem;
            let nextItem;
            const paginationItemClasses = index$1.cx(index.prefix('pagination__item'));
            paginationModel.forEach((pageModel) => {
                if (pageModel.type === itemTypes.PREVIOUS_PAGE_LINK) {
                    const paginationPrevClasses = index$1.cx(index.prefix('pagination__prev'), !pageModel.isActive && index.prefix('pagination__prev--disabled'));
                ...
                if (pageModel.type === itemTypes.ELLIPSIS) {
                    const paginationGoToClasses = index$1.cx(index.prefix('pagination__goto'), index.prefix('pagination__goto--ellipsis'));
                ...
                if (pageModel.type === itemTypes.PAGE) {
                    const paginationGoToClasses = index$1.cx(index.prefix('pagination__goto'), pageModel.isActive && index.prefix('pagination__goto--current'));
                ...
                if (pageModel.type === itemTypes.NEXT_PAGE_LINK) {
                    const paginationNextClasses = index$1.cx(index.prefix('pagination__next'), !pageModel.isActive && index.prefix('pagination__next--disabled'));
