...
const Icon = class {
    ...
    render() {
        const iconClasses = index$1.cx(index.prefix('icon'), index.prefix(`icon--size-${this.size}`), index.prefix(`icon--color-${this.color}`), this.color !== 'inherit' && index.prefix(`icon--theme-${this.theme}`));
        return (index.h(index.Host, null, index.h("i", { class: iconClasses, innerHTML: this.svgContent })));
    }
...
const Text = class {
    ...
    render() {
        ...
        const textClasses = index$1.cx(index.prefix('text'), index.mapBreakpointPropToPrefixedClasses('text--size', this.size), index.prefix(`text--weight-${this.weight}`), index.prefix(`text--align-${this.align}`), index.prefix(`text--color-${this.color}`), this.ellipsis && index.prefix('text--ellipsis'), this.color !== 'inherit' && index.prefix(`text--theme-${this.theme}`));
    ...
    bindFontSizeListener() {
        index.transitionListener(this.textTag, 'font-size', () => {
            this.textTag.style.lineHeight = `${index.calcLineHeightForElement(this.textTag)}`;
        });
    }
    addSlottedStyles() {
