...
const index$1 = require('./index-7a33061a.js');
...
function pascalCase(input, options) {
    if (options === void 0) { options = {}; }
    return noCase(input, breakpointCustomizable.__assign({ delimiter: "", transform: pascalCaseTransform }, options));
}
...
function camelCase(input, options) {
    if (options === void 0) { options = {}; }
    return pascalCase(input, breakpointCustomizable.__assign({ transform: camelCaseTransform }, options));
}
...
const buildIconUrl = (iconNameOrSource = DEFAULT_ICON_NAME) => {
    const cdnBaseUrl =  index$1.ICONS_CDN_BASE_URL ;
    ...
    else if (index$1.ICONS_MANIFEST$1[camelCase(iconNameOrSource)]) {
        // check if IconName exists
        return `${cdnBaseUrl}/${index$1.ICONS_MANIFEST$1[camelCase(iconNameOrSource)]}`;
    }
...
class Icon {
    ...
    render() {
        const iconClasses = index.classnames(breakpointCustomizable.prefix('icon'), breakpointCustomizable.prefix(`icon--size-${this.size}`), breakpointCustomizable.prefix(`icon--color-${this.color}`), this.color !== 'inherit' && breakpointCustomizable.prefix(`icon--theme-${this.theme}`));
        return (breakpointCustomizable.h(breakpointCustomizable.Host, null, breakpointCustomizable.h("i", { class: iconClasses, innerHTML: this.svgContent })));
    }
    ...
    get el() { return breakpointCustomizable.getElement(this); }
...
class Text {
    ...
    render() {
        ...
        const textClasses = index.classnames(breakpointCustomizable.prefix('text'), breakpointCustomizable.mapBreakpointPropToPrefixedClasses('text--size', this.size), breakpointCustomizable.prefix(`text--weight-${this.weight}`), breakpointCustomizable.prefix(`text--align-${this.align}`), breakpointCustomizable.prefix(`text--color-${this.color}`), this.ellipsis && breakpointCustomizable.prefix('text--ellipsis'), this.color !== 'inherit' && breakpointCustomizable.prefix(`text--theme-${this.theme}`));
        return (breakpointCustomizable.h(TagType, { class: textClasses, ref: el => this.textTag = el }, breakpointCustomizable.h("slot", null)));
    }
    ...
    bindFontSizeListener() {
        transitionListener.transitionListener(this.textTag, 'font-size', () => {
            this.textTag.style.lineHeight = `${lineHeight.calcLineHeightForElement(this.textTag)}`;
        });
    }
    addSlottedStyles() {
