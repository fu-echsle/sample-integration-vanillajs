...
const lineHeight = require('./line-height-8dc9c7e5.js');
...
class ButtonPure {
    ...
    componentDidLoad() {
        focusHandling.improveFocusHandlingForCustomElement(this.host);
        buttonHandling.improveButtonHandlingForCustomElement(this.host, () => this.type, () => this.isDisabled());
        transitionListener.transitionListener(this.buttonTag, 'font-size', () => {
            const size = lineHeight.calcLineHeightForElement(this.buttonTag);
            this.iconTag.style.width = `${size}em`;
            this.iconTag.style.height = `${size}em`;
        });
    }
    render() {
        const buttonPureClasses = index.classnames(breakpointCustomizable.prefix('button-pure'), breakpointCustomizable.mapBreakpointPropToPrefixedClasses('button-pure--size', this.size), breakpointCustomizable.prefix(`button-pure--theme-${this.theme}`));
        const iconClasses = index.classnames(breakpointCustomizable.prefix('button-pure__icon'));
        const labelClasses = index.classnames(breakpointCustomizable.prefix('button-pure__label'), breakpointCustomizable.mapBreakpointPropToPrefixedClasses('button-pure__label-', this.hideLabel, ['hidden', 'visible']));
        const sublineClasses = index.classnames(breakpointCustomizable.prefix('button-pure__subline'), breakpointCustomizable.mapBreakpointPropToPrefixedClasses('button-pure__subline-', this.hideLabel, ['hidden', 'visible']));
        return (breakpointCustomizable.h(breakpointCustomizable.Host, null, breakpointCustomizable.h("button", { class: buttonPureClasses, type: this.type, disabled: this.isDisabled(), tabindex: this.tabbable ? 0 : -1, ref: (el) => (this.buttonTag = el), "aria-busy": this.loading && 'true' }, this.loading ? (breakpointCustomizable.h("p-spinner", { class: iconClasses, size: "inherit", theme: this.theme, ref: (el) => (this.iconTag = el) })) : (breakpointCustomizable.h("p-icon", { class: iconClasses, color: "inherit", size: "inherit", name: this.icon, source: this.iconSource, ref: (el) => (this.iconTag = el), "aria-hidden": "true" })), breakpointCustomizable.h("p-text", { class: labelClasses, tag: "span", color: "inherit", size: "inherit", weight: this.weight }, breakpointCustomizable.h("slot", null))), this.hasSubline && (breakpointCustomizable.h("p-text", { class: sublineClasses, color: "inherit", size: "inherit", tag: "div" }, breakpointCustomizable.h("slot", { name: "subline" })))));
    }
    checkHasSubline() {
