...
const Link = class {
    ...
    `;
        index.insertSlottedStyles(this.element, style);
        focusHandling.improveFocusHandlingForCustomElement(this.element);
    }
    render() {
        const TagType = this.href === undefined ? 'span' : 'a';
        const linkClasses = index$1.cx(index.prefix('link'), index.prefix(`link--${this.variant}`), index.mapBreakpointPropToPrefixedClasses('link-', this.hideLabel, ['without-label', 'with-label']), index.prefix(`link--theme-${this.theme}`));
        const iconClasses = index.prefix('link__icon');
        const labelClasses = index.prefix('link__label');
