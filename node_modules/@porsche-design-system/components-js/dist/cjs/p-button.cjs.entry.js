...
const Button = class {
    ...
    componentDidLoad() {
        ...
        index.improveButtonHandlingForCustomElement(this.element, () => this.type, () => this.isDisabled());
    }
    render() {
        const buttonClasses = index$1.cx(index.prefix('button'), index.prefix(`button--${this.variant}`), index.mapBreakpointPropToPrefixedClasses('button-', this.hideLabel, ['without-label', 'with-label']), index.prefix(`button--theme-${this.theme}`));
        const iconClasses = index.prefix('button__icon');
        const labelClasses = index.prefix('button__label');
        return (index.h("button", { class: buttonClasses, type: this.type, disabled: this.isDisabled(), tabindex: this.tabbable ? 0 : -1, "aria-busy": this.loading && 'true' }, this.loading ? (index.h("p-spinner", { class: iconClasses, size: "inherit", theme: (this.variant === 'tertiary' && this.theme) || 'dark' })) : (index.h("p-icon", { class: iconClasses, size: "inherit", name: this.icon, source: this.iconSource, color: "inherit", "aria-hidden": "true" })), index.h("p-text", { class: labelClasses, tag: "span", color: "inherit" }, index.h("slot", null))));
    }
    isDisabled() {
