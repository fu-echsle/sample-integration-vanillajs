...
const Button = class {
    ...
    render() {
        const buttonClasses = index$1.cx(index$1.prefix('button'), index$1.prefix(`button--${this.variant}`), index$1.mapBreakpointPropToPrefixedClasses('button-', this.hideLabel, ['without-label', 'with-label']), index$1.prefix(`button--theme-${this.theme}`));
        const iconClasses = index$1.prefix('button__icon');
        const labelClasses = index$1.prefix('button__label');
        return (index.h("button", { class: buttonClasses, type: this.type, disabled: this.isDisabled(), tabindex: this.tabbable ? 0 : -1, "aria-busy": this.loading && 'true' }, this.loading ? (index.h("p-spinner", { class: iconClasses, size: 'inherit', theme: this.variant === 'tertiary' && this.theme === 'light' ? 'light' : 'dark' })) : (index.h("p-icon", { class: iconClasses, size: 'inherit', name: this.icon, source: this.iconSource, color: 'inherit', "aria-hidden": 'true' })), index.h("p-text", { class: labelClasses, tag: 'span', color: 'inherit' }, index.h("slot", null))));
    }
    isDisabled() {
