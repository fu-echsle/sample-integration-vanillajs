...
const TextListItem = class {
    ...
    render() {
        const textListItemClasses = index$1.cx(index.prefix('text-list-item'), index.prefix(`text-list-item--${this.typeOfList}`), {
            [index.prefix(`text-list-item--ordered-${this.typeOfOrderedList}`)]: this.typeOfList === 'ordered',
            [index.prefix('text-list-item--nested')]: this.isNestedList
        });
        return (index.h(index.Host, { role: 'listitem', class: textListItemClasses }, index.h("slot", null)));
    }
    get typeOfList() {
        const list = this.host.closest(index.prefix('text-list'));
    ...
    get isNestedList() {
        return !!this.host.closest(index.prefix('text-list[nested]'));
    }
    addSlottedStyles() {
