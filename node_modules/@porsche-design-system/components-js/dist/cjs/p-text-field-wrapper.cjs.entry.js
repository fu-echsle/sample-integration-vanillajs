...
const buttonHandling = require('./buttonHandling-960956e8.js');
...
class TextFieldWrapper {
    constructor(hostRef) {
        ...
        this.onSubmitHandler = (event) => {
            if (this.isInputTypeSearch) {
                buttonHandling.handleButtonEvent(event, this.host, () => this.searchButtonType, () => this.disabled);
            }
        };
    }
    componentWillLoad() {
    ...
    render() {
        const containerClasses = index.classnames(breakpointCustomizable.prefix('text-field-wrapper__container'));
        const labelClasses = index.classnames(breakpointCustomizable.prefix('text-field-wrapper__label'));
        const labelTextClasses = index.classnames(breakpointCustomizable.prefix('text-field-wrapper__label-text'), breakpointCustomizable.mapBreakpointPropToPrefixedClasses('text-field-wrapper__label-text-', this.hideLabel, ['hidden', 'visible']), this.disabled && breakpointCustomizable.prefix('text-field-wrapper__label-text--disabled'));
        const descriptionTextClasses = index.classnames(breakpointCustomizable.prefix('text-field-wrapper__description-text'), breakpointCustomizable.mapBreakpointPropToPrefixedClasses('text-field-wrapper__description-text-', this.hideLabel, [
            'hidden',
            'visible'
        ]), this.disabled && breakpointCustomizable.prefix('text-field-wrapper__description-text--disabled'));
        const fakeInputClasses = index.classnames(breakpointCustomizable.prefix('text-field-wrapper__fake-input'), this.state !== 'none' && breakpointCustomizable.prefix(`text-field-wrapper__fake-input--${this.state}`), this.disabled && breakpointCustomizable.prefix('text-field-wrapper__fake-input--disabled'), this.readonly && breakpointCustomizable.prefix('text-field-wrapper__fake-input--readonly'));
        const buttonClasses = index.classnames(breakpointCustomizable.prefix('text-field-wrapper__button'));
        const messageClasses = index.classnames(breakpointCustomizable.prefix('text-field-wrapper__message'), this.state !== 'none' && breakpointCustomizable.prefix(`text-field-wrapper__message--${this.state}`));
        return (breakpointCustomizable.h(breakpointCustomizable.Host, null, breakpointCustomizable.h("div", { class: containerClasses }, breakpointCustomizable.h("label", { class: labelClasses }, this.isLabelVisible && (breakpointCustomizable.h("p-text", { class: labelTextClasses, tag: "span", color: "inherit", onClick: this.labelClick }, this.label || (breakpointCustomizable.h("span", null, breakpointCustomizable.h("slot", { name: "label" }))))), this.isDescriptionVisible && (breakpointCustomizable.h("p-text", { class: descriptionTextClasses, tag: "span", color: "inherit", size: "x-small", onClick: this.labelClick }, this.description || (breakpointCustomizable.h("span", null, breakpointCustomizable.h("slot", { name: "description" }))))), breakpointCustomizable.h("span", { class: fakeInputClasses }, breakpointCustomizable.h("slot", null))), this.isPasswordToggleable && (breakpointCustomizable.h("button", { type: "button", class: buttonClasses, onClick: this.togglePassword, disabled: this.disabled }, breakpointCustomizable.h("p-icon", { name: this.showPassword ? 'view-off' : 'view', color: "inherit" }))), this.isInputTypeSearch && (breakpointCustomizable.h("button", { onClick: this.onSubmitHandler, type: "submit", class: buttonClasses, disabled: this.disabled || this.readonly }, breakpointCustomizable.h("p-icon", { name: "search", color: "inherit" })))), this.isMessageVisible && (breakpointCustomizable.h("p-text", { class: messageClasses, color: "inherit", role: this.state === 'error' && 'alert' }, this.message || (breakpointCustomizable.h("span", null, breakpointCustomizable.h("slot", { name: "message" })))))));
    }
    get isLabelVisible() {
    ...
    bindStateListener() {
        transitionListener.transitionListener(this.input, 'border-top-color', this.setState);
    }
