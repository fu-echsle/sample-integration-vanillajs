...
const stencilSubscription = ({ on }) => {
    ...
    if (typeof _commonjsHelpers.getRenderingRef === 'function') {
        ...
        on('get', (propName) => {
            const elm = _commonjsHelpers.getRenderingRef();
            if (elm) {
        ...
        on('set', (propName) => {
            ...
            if (elements) {
                elmsToUpdate.set(propName, elements.filter(_commonjsHelpers.forceUpdate));
            }
            cleanupElements(elmsToUpdate);
        });
        on('reset', () => {
            elmsToUpdate.forEach((elms) => elms.forEach(_commonjsHelpers.forceUpdate));
            cleanupElements(elmsToUpdate);
        });
    }
};
...
const DEFAULT_ACCESSIBILITY_STATEMENT = null;
const DEFAULT_FURTHER_LINKS = null;
const DEFAULT_SERIES_LIST = [];
const DEFAULT_SHOPS = [];
const DEFAULT_MORE = [];
const DEFAULT_METAFUNCTIONALITIES = [];
const DEFAULT_MENU = [];
const DEFAULT_ENVIRONMENT = 'production';
const ANALYTICS_APP_ID = 'nav';
const EMPTY_SERIES = {
    PhraseId: '',
    Name: '',
    Image: '',
...
const EMPTY_ITEM = { PhraseId: '', Text: '' };
...
const DEFAULT_EMPTY_CONTENT_RESPONSE = [];
const DEFAULT_EMPTY_CONTENT = {
    accessibilityStatement: DEFAULT_ACCESSIBILITY_STATEMENT,
    furtherLinks: DEFAULT_FURTHER_LINKS,
...
const ACCESS_TOKEN_FETCHED_EVENT = 'accessTokenFetched';
const ACCESS_TOKEN_FETCH_ERROR_EVENT = 'accessTokenFetchError';
const TEXT_FIELD_NAME = 'Text';
const LINK_FIELD_NAME = 'Link';
const PHRASEID_FIELD_NAME = 'PhraseId';
const DEALER_SEARCH_PHRASE_ID = 'metafunctionalities.finddealer';
const MYPORSCHE_PHRASE_ID = 'metafunctionalities.myporsche';
const CREST_PHRASE_ID = 'mainmenu.crest';
const MENU_PHRASE_ID = 'mainmenu.menu';
const FEATURE_OVERRIDE_ENABLED = 'featureOverrideEnabled';
const FEATURE_MODEL_META = 'homenav_1163_modelMeta';
const FEATURE_MOBILE_FOCUS = 'homenav_1174_focusMgmt';
const FEATURE_NEW_LEVEL2 = 'homenav_1297_newLevel2';
const FEATURE_ACCESSIBILITY_STATEMENT = 'homenav_1312_accessibilityStatement';
const MAX_M_VIEWPORT_WIDTH = 1299;
const FOCUSABLE_ELEMENTS_SELECTOR = 'a, button, [tabindex]:not([tabindex="-1"]), phn-p-link-pure, phn-p-button-pure, phn-p-link';
...
const analyticsSetupStore = createStore({
...
class MenuBackClickEvent extends AnalyticsEvent {
    constructor(analyticsSetup, contentItem) {
        super(analyticsSetup);
        this.initComponentClick();
        this.context.eventAction = 'PAGHomeNav_MenuBack_Click';
        this.populateComponentClickFromContentItem(contentItem);
    }
    initComponentClick() {
        this.componentClick = {
    ...
    populateComponentClickFromContentItem(contentItem) {
    ...
    event() {
        ...
        return Object.assign(Object.assign({}, super.event()), { componentClick });
    }
}

const backButtonCss = ".sc-phn-back-button-h{font-size:1rem;display:-ms-flexbox;display:flex}@media (min-width: 1300px){.sc-phn-back-button-h{visibility:hidden}}@media (min-width: 760px) and (max-width: 999px){.sc-phn-back-button-h{font-size:1.125rem}}";
...
const BackButton = class {
    constructor(hostRef) {
        ...
        this.text = '';
        this.buttonId = '';
        this.sendMenuBackClickEventAnalyticsEvent = () => {
            const contentItem = { Text: this.text, PhraseId: this.buttonId, Link: '' };
            ...
            const menuBackClickEvent = new MenuBackClickEvent(analyticsSetup, contentItem);
            const pagData = (window['pagData'] = window['pagData'] || []);
            pagData.push(menuBackClickEvent.event());
        };
    }
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("phn-p-button-pure", { size: "inherit", icon: "arrow-head-left", class: "back-button-text-link", "aria-label": "Back to Menu", onClick: this.sendMenuBackClickEventAnalyticsEvent }, this.text)));
    }
};
BackButton.style = backButtonCss;

const navContentStore = createStore({
    accessibilityStatement: DEFAULT_ACCESSIBILITY_STATEMENT,
    furtherLinks: DEFAULT_FURTHER_LINKS,
    seriesList: DEFAULT_SERIES_LIST,
    shops: DEFAULT_SHOPS,
...
class MoreClickEvent extends AnalyticsEvent {
    constructor(analyticsSetup, contentItem) {
        super(analyticsSetup);
        this.initComponentClick();
        this.context.eventAction = 'PAGHomeNav_More_Click';
        this.populateComponentClickFromContentItem(contentItem);
    }
    initComponentClick() {
        this.componentClick = {
...
const local$1 = {
	...
	homenav_1297_newLevel2: true,
	homenav_1312_accessibilityStatement: true
};
const preview$1 = {
	featureOverrideEnabled: true,
	homenav_feature1: false,
...
const config = (env = DEFAULT_ENVIRONMENT) => configJson[env] || configJson['local'];
...
const isFeatureEnabled = (feature, env = DEFAULT_ENVIRONMENT) => {
...
const carFrontshotCss = ".frontshot.sc-phn-car-frontshot{width:100%}@media (min-width: 1300px) and (max-width: 1759px){.frontshot.sc-phn-car-frontshot{padding-bottom:1.5625rem}}";

const CarFrontshot = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    }
    render() {
        const path = _commonjsHelpers.getAssetPath(`${config(navContentStore.get('env')).ASSETS_URL}/frontshot-assets`);
        const imageFileEnding = 'jpg';
        const imageType = 'jpeg';
        return (_commonjsHelpers.h("picture", { "aria-hidden": "true" }, _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 1760px)", srcSet: `${path}/1920/${this.image}.${imageFileEnding}, ${path}/1920/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 1024px)", srcSet: `${path}/1366/${this.image}.${imageFileEnding}, ${path}/1366/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 768px)", srcSet: `${path}/1920/${this.image}.${imageFileEnding}, ${path}/1920/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 0px)", srcSet: `${path}/375/${this.image}.${imageFileEnding}, ${path}/375/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("img", { src: `${path}/375/${this.image}.${imageFileEnding}`, class: "frontshot", alt: this.text })));
    }
    static get assetsDirs() { return ["frontshot-assets"]; }
};
CarFrontshot.style = carFrontshotCss;

class NavigationClickEvent extends AnalyticsEvent {
    constructor(analyticsSetup, contentItem) {
        super(analyticsSetup);
        this.context.eventAction = 'PAGHomeNav_Navigation_Click';
        this.initComponentClick();
        this.populateComponentClickFromContentItem(contentItem);
    }
    initComponentClick() {
...
const carFurtherLinksCss = "@media (min-width: 1300px){.car-further-links.sc-phn-car-further-links{display:-ms-flexbox;display:flex;-ms-flex-pack:center;justify-content:center}}.car-further-links.sc-phn-car-further-links .car-further-links-wrapper.sc-phn-car-further-links{border-bottom:1px solid #e3e4e5;margin-bottom:3.75rem}@media (min-width: 1300px){.car-further-links.sc-phn-car-further-links .car-further-links-wrapper.sc-phn-car-further-links{border-top:1px solid #e3e4e5;display:-ms-flexbox;display:flex;-ms-flex-direction:row;flex-direction:row;margin-bottom:unset}}.car-further-links-item.sc-phn-car-further-links{display:-ms-flexbox;display:flex;-ms-flex-direction:row;flex-direction:row;font-weight:normal;-ms-flex-align:center;align-items:center;border-top:1px solid #e3e4e5;-webkit-box-sizing:border-box;box-sizing:border-box;font-size:1rem;padding:15px 0}.car-further-links-item.sc-phn-car-further-links:first-child{border-top:0}@media (min-width: 760px) and (max-width: 999px){.car-further-links-item.sc-phn-car-further-links{font-size:1.125rem;padding:18px 0}}@media (min-width: 1300px){.car-further-links-item.sc-phn-car-further-links{height:1.5rem;padding:0.875rem 0.625rem;border-top:0;-webkit-box-sizing:unset;box-sizing:unset}.car-further-links-item.sc-phn-car-further-links:first-child{margin-right:0.25rem}}";

const CarFurtherLinks = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.items = [
            { PhraseId: '', Text: '', Link: '' },
    ...
    renderItem(item) {
        return (_commonjsHelpers.h("phn-p-link-pure", { href: item.Link, icon: 'arrow-head-right', size: "inherit", class: "car-further-links-item", onClick: () => this.sendNavigationClickAnalyticsEvent(item), role: "button" }, item.Text));
    }
    render() {
        return (_commonjsHelpers.h("div", { class: "car-further-links" }, _commonjsHelpers.h("div", { class: "car-further-links-wrapper" }, this.renderItem(this.items[0]), this.renderItem(this.items[1]))));
    }
};
...
const carFurtherLinksV2Css = ".car-further-links.sc-phn-car-further-links-v2{border-top:1px solid #e3e4e5}@media (min-width: 1300px) and (max-width: 1759px){.car-further-links.sc-phn-car-further-links-v2{margin-bottom:-5rem}}@media (min-width: 1760px){.car-further-links.sc-phn-car-further-links-v2{margin-bottom:-6rem}}.car-further-links.sc-phn-car-further-links-v2 .limit-stretch.sc-phn-car-further-links-v2{width:calc(100vw - 7vw * 2);max-width:96rem;margin-left:auto;margin-right:auto;-webkit-box-sizing:content-box;box-sizing:content-box}@media (min-width: 1760px){.car-further-links.sc-phn-car-further-links-v2 .limit-stretch.sc-phn-car-further-links-v2{width:calc(100vw - 10vw * 2)}}@media (min-width: 1000px){.car-further-links.sc-phn-car-further-links-v2 .limit-stretch.sc-phn-car-further-links-v2{padding-left:calc(100vw - 100%)}}.car-further-links.sc-phn-car-further-links-v2 .car-further-links-item.sc-phn-car-further-links-v2{padding:1.125rem 0 1.125rem 0.8125rem;display:-ms-flexbox;display:flex}";

const CarFurtherLinksV2 = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "car-further-links" }, _commonjsHelpers.h("div", { class: this.isDesktop ? 'limit-stretch' : '' }, _commonjsHelpers.h("phn-p-grid", { direction: "row-reverse" }, _commonjsHelpers.h("phn-p-grid-item", { size: "{base: 12, l:3}" }, _commonjsHelpers.h("phn-p-link-pure", { href: this.furtherLinks.Link, icon: 'arrow-head-right', size: "inherit", class: "car-further-links-item", onClick: () => this.sendNavigationClickAnalyticsEvent(this.furtherLinks) }, this.furtherLinks.Text))))));
    }
};
CarFurtherLinksV2.style = carFurtherLinksV2Css;

class ModelAnalyticsEvent extends AnalyticsEvent {
...
class ConfigureClickEvent extends ModelAnalyticsEvent {
    constructor(analyticsSetup, modelSeriesName, contentItem) {
        super(analyticsSetup, modelSeriesName);
        this.context.eventAction = 'PAGHomeNav_Configure_Click';
        this.populateComponentClickFromLink(contentItem);
    }
    populateComponentClickFromLink(contentItem) {
        this.componentClick.clickElementId = contentItem.PhraseId;
        this.componentClick.clickElementName = contentItem.Text;
...
class CompareClickEvent extends ModelAnalyticsEvent {
    constructor(analyticsSetup, modelSeriesName, contentItem) {
        super(analyticsSetup, modelSeriesName);
        this.context.eventAction = 'PAGHomeNav_Compare_Click';
        this.populateComponentClickFromLink(contentItem);
    }
    populateComponentClickFromLink(contentItem) {
        this.componentClick.clickElementId = contentItem.PhraseId;
...
class FindCarClickEvent extends ModelAnalyticsEvent {
    constructor(analyticsSetup, modelSeriesName, contentItem) {
        super(analyticsSetup, modelSeriesName);
        this.context.eventAction = 'PAGHomeNav_FindCar_Click';
        this.populateComponentClickFromLink(contentItem);
    }
    populateComponentClickFromLink(contentItem) {
        this.componentClick.clickElementId = contentItem.PhraseId;
...
class ModelMetaClickEvent extends ModelAnalyticsEvent {
    constructor(analyticsSetup, modelRangeName, modelMeta) {
        super(analyticsSetup, modelRangeName);
        this.context.eventAction = 'PAGHomeNav_ModelMetaFunction_Click';
        this.populateComponentClickFromLink(modelMeta);
    }
    populateComponentClickFromLink(modelMeta) {
...
const carMetaFunctionalitiesCss = "@media (min-width: 1300px) and (max-width: 1759px){.car-meta-functionalities.sc-phn-car-meta-functionalities{display:-ms-flexbox;display:flex;-ms-flex-pack:center;justify-content:center;padding-top:3.125rem}}@media (min-width: 1760px){.car-meta-functionalities.sc-phn-car-meta-functionalities{display:-ms-flexbox;display:flex;-ms-flex-pack:center;justify-content:center;padding-top:3rem}}.car-meta-functionalities.sc-phn-car-meta-functionalities .car-meta-functionalities-wrapper.sc-phn-car-meta-functionalities{border-bottom:1px solid #e3e4e5;border-top:1px solid #e3e4e5}@media (min-width: 0) and (max-width: 1299px){.car-meta-functionalities.sc-phn-car-meta-functionalities .car-meta-functionalities-wrapper.sc-phn-car-meta-functionalities{border:0}}@media (min-width: 1300px){.car-meta-functionalities.sc-phn-car-meta-functionalities .car-meta-functionalities-wrapper.sc-phn-car-meta-functionalities{display:-ms-flexbox;display:flex;-ms-flex-direction:row;flex-direction:row;-ms-flex-pack:center;justify-content:center;height:3.375rem}}.car-meta-functionalities-item.sc-phn-car-meta-functionalities{display:-ms-flexbox;display:flex;-ms-flex-direction:row;flex-direction:row;-ms-flex-align:center;align-items:center}@media (min-width: 0) and (max-width: 759px){.car-meta-functionalities-item.sc-phn-car-meta-functionalities{font-size:1rem;padding:0.875rem 0 0.875rem 0;border-bottom:1px solid #e3e4e5}}@media (min-width: 760px) and (max-width: 999px){.car-meta-functionalities-item.sc-phn-car-meta-functionalities{font-size:1.125rem;padding:1.125rem 0 1.125rem 0;border-bottom:1px solid #e3e4e5}}@media (min-width: 1000px) and (max-width: 1299px){.car-meta-functionalities-item.sc-phn-car-meta-functionalities{font-size:1rem;padding:0.875rem 0 0.875rem 0;border-bottom:1px solid #e3e4e5}}@media (min-width: 1300px){.car-meta-functionalities-item.sc-phn-car-meta-functionalities{font-size:1rem;padding:0.875rem 0 0.875rem 1.5rem;border-bottom:0}.car-meta-functionalities-item.sc-phn-car-meta-functionalities:nth-last-child(1){margin-right:0.625rem}.car-meta-functionalities-item.sc-phn-car-meta-functionalities:nth-child(1){padding:0.875rem 0 0.875rem 0.625rem}}";

const CarMetaFunctionalities = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.items = [
            { PhraseId: '', Text: '', Link: '' },
    ...
    renderItem(source, contentItem, analyticsEvent) {
        return (_commonjsHelpers.h("phn-p-link-pure", { href: contentItem.Link, icon: source, size: "inherit", onClick: () => analyticsEvent(contentItem), class: "car-meta-functionalities-item" }, contentItem.Text));
    }
    render() {
        return isFeatureEnabled(FEATURE_MODEL_META) ? (_commonjsHelpers.h("div", { class: "car-meta-functionalities" }, _commonjsHelpers.h("div", { class: "car-meta-functionalities-wrapper" }, this.modelMetaItems.map((item) => (_commonjsHelpers.h("phn-p-link-pure", { href: item.Link, icon: item.Icon, onClick: () => this.sendModelMetaFunctionAnalyticsEvent(item), size: "inherit", class: "car-meta-functionalities-item" }, item.Text)))))) : (_commonjsHelpers.h("div", { class: "car-meta-functionalities" }, _commonjsHelpers.h("div", { class: "car-meta-functionalities-wrapper" }, this.renderItem('configurate', this.items[0], this.sendConfigureClickAnalyticsEvent), this.renderItem('compare', this.items[1], this.sendCompareClickAnalyticsEvent), this.renderItem('search', this.items[2], this.sendFindCarClickAnalyticsEvent))));
    }
};
CarMetaFunctionalities.style = carMetaFunctionalitiesCss;

const carMetaFunctionalitiesV2Css = "@media (min-width: 1300px){.car-meta-functionalities.sc-phn-car-meta-functionalities-v2{padding-top:3.5rem}}@media (min-width: 1760px){.car-meta-functionalities.sc-phn-car-meta-functionalities-v2{padding-top:5rem}}@media (min-width: 1760px){.car-meta-functionalities.sc-phn-car-meta-functionalities-v2{padding-top:5rem}}.car-meta-functionalities.sc-phn-car-meta-functionalities-v2 .car-meta-functionalities-wrapper.sc-phn-car-meta-functionalities-v2{border-top:1px solid #e3e4e5}@media (min-width: 0) and (max-width: 1299px){.car-meta-functionalities.sc-phn-car-meta-functionalities-v2 .car-meta-functionalities-wrapper.sc-phn-car-meta-functionalities-v2{border:0}}@media (min-width: 1300px){.car-meta-functionalities.sc-phn-car-meta-functionalities-v2 .car-meta-functionalities-wrapper.sc-phn-car-meta-functionalities-v2{display:-ms-flexbox;display:flex;-ms-flex-direction:column;flex-direction:column;-ms-flex-pack:left;justify-content:left}}.car-meta-functionalities-item.sc-phn-car-meta-functionalities-v2{display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center;border-bottom:1px solid #e3e4e5;padding:0.9375rem 0 0.9375rem 0;font-size:1rem}@media (min-width: 760px) and (max-width: 1299px){.car-meta-functionalities-item.sc-phn-car-meta-functionalities-v2{width:100%;font-size:1.125rem;padding:1.125rem 0 1.125rem 0}}@media (min-width: 0) and (max-width: 1299px){.car-meta-functionalities-item.sc-phn-car-meta-functionalities-v2{padding-left:0.75rem;-webkit-box-sizing:border-box;box-sizing:border-box}}";

const CarMetaFunctionalitiesV2 = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.items = [
            { PhraseId: '', Text: '', Link: '' },
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "car-meta-functionalities" }, _commonjsHelpers.h("div", { class: "car-meta-functionalities-wrapper" }, this.modelMetaItems.map((item) => (_commonjsHelpers.h("phn-p-link-pure", { class: "car-meta-functionalities-item", size: "inherit", href: item.Link, icon: item.Icon, onClick: () => this.sendModelMetaFunctionAnalyticsEvent(item) }, item.Text))))));
    }
};
CarMetaFunctionalitiesV2.style = carMetaFunctionalitiesV2Css;

const carModelCss = ".car-model.sc-phn-car-model{text-decoration:none;color:#000;padding-top:0.6875rem;padding-bottom:0.875rem;display:-ms-inline-flexbox;display:inline-flex;-ms-flex-direction:row;flex-direction:row;-ms-flex-align:center;align-items:center;width:100%;text-align:left;border-bottom:1px solid #e3e4e5}@media (min-width: 760px){.car-model.sc-phn-car-model{padding-top:unset;padding-bottom:unset}}@media (min-width: 1000px){.car-model.sc-phn-car-model:focus{outline-width:2px;outline-style:solid;outline-offset:1px;outline-color:currentColor;outline-offset:-2px}}@media (min-width: 1300px){.car-model.sc-phn-car-model{display:block;-ms-flex-direction:column;flex-direction:column;-ms-flex-align:unset;align-items:unset;width:unset;text-align:center;border-bottom:0}}.car-model.sc-phn-car-model:hover .model-text.sc-phn-car-model{color:#d5001c}.car-model.sc-phn-car-model .model-pic.sc-phn-car-model{width:50%}.car-model.sc-phn-car-model .model-pic.sc-phn-car-model img.sc-phn-car-model{vertical-align:middle}@media (min-width: 760px) and (max-width: 999px){.car-model.sc-phn-car-model .model-pic.sc-phn-car-model{height:10.875rem}}@media (min-width: 1000px) and (max-width: 1299px){.car-model.sc-phn-car-model .model-pic.sc-phn-car-model{height:8.125rem}}@media (min-width: 1300px){.car-model.sc-phn-car-model .model-pic.sc-phn-car-model{width:100%}}.car-model.sc-phn-car-model .model-text.sc-phn-car-model{width:50%}@media (min-width: 0) and (max-width: 759px){.car-model.sc-phn-car-model .model-text.sc-phn-car-model{font-size:1rem;line-height:1.5rem}}@media (min-width: 760px) and (max-width: 999px){.car-model.sc-phn-car-model .model-text.sc-phn-car-model{font-size:1.875rem;line-height:2.5rem}}@media (min-width: 1000px) and (max-width: 1299px){.car-model.sc-phn-car-model .model-text.sc-phn-car-model{font-size:1.5rem;line-height:2rem}}@media (min-width: 1300px){.car-model.sc-phn-car-model .model-text.sc-phn-car-model{width:100%;margin-top:-0.9375rem}}";
...
const CarModel = class {
    ...
    render() {
        const path = _commonjsHelpers.getAssetPath(`${config(navContentStore.get('env')).ASSETS_URL}/model-assets`);
        const imageFileEnding = 'jpg';
        const imageType = 'jpeg';
        return (_commonjsHelpers.h("a", { class: "car-model", href: this.link }, _commonjsHelpers.h("picture", { class: "model-pic", "aria-hidden": "true" }, _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 1760px)", srcSet: `${path}/1920/${this.image}.${imageFileEnding}, ${path}/1920/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 1300px)", srcSet: `${path}/1366/${this.image}.${imageFileEnding}, ${path}/1366/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 1000px)", srcSet: `${path}/1024/${this.image}.${imageFileEnding}, ${path}/1024/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 760px)", srcSet: `${path}/768/${this.image}.${imageFileEnding}, ${path}/768/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 375px)", srcSet: `${path}/375/${this.image}.${imageFileEnding}, ${path}/375/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("source", { type: `image/${imageType}`, media: "(min-width: 0px)", srcSet: `${path}/320/${this.image}.${imageFileEnding}, ${path}/320/${this.image}@2x.${imageFileEnding} 2x` }), _commonjsHelpers.h("img", { alt: this.text })), _commonjsHelpers.h("div", { class: "model-text" }, this.text)));
    }
    static get assetsDirs() { return ["model-assets"]; }
};
CarModel.style = carModelCss;

class ModelClickEvent extends ModelAnalyticsEvent {
    constructor(analyticsSetup, modelRangeName, model) {
        super(analyticsSetup, modelRangeName);
        this.context.eventAction = 'PAGHomeNav_Model_Click';
        this.populateComponentClickFromLink(model);
    }
    populateComponentClickFromLink(model) {
...
const CarModelList = class {
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "car-model-list" }, _commonjsHelpers.h("div", { class: 'car-model-list-wrapper car-models-count-' + this.series.Models.length }, this.series.Models.map((model, index) => (_commonjsHelpers.h("div", { class: 'car-model car-model-' + index }, _commonjsHelpers.h("phn-car-model", { text: model.Name, image: model.Image, link: model.Link, onClick: () => this.sendModelClickAnalyticsEvent(model) })))))));
    }
};
CarModelList.style = carModelListCss;

const carModelListV2Css = ".car-model-list.sc-phn-car-model-list-v2{display:-ms-flexbox;display:flex;-ms-flex-pack:center;justify-content:center;-ms-flex-direction:column;flex-direction:column;margin-top:1.5rem;margin-bottom:3rem}@media (min-width: 376px){.car-model-list.sc-phn-car-model-list-v2{margin-top:2.25rem;margin-bottom:3.75rem}}@media (min-width: 1300px){.car-model-list.sc-phn-car-model-list-v2{margin-top:3.5rem}}@media (min-width: 1760px){.car-model-list.sc-phn-car-model-list-v2{margin-top:5rem}}.car-model-list.sc-phn-car-model-list-v2 .car-model-list-item.sc-phn-car-model-list-v2{margin-bottom:0.5rem}@media (min-width: 376px) and (max-width: 1299px){.car-model-list.sc-phn-car-model-list-v2 .car-model-list-item.sc-phn-car-model-list-v2{margin-bottom:1rem}}.car-model-list.sc-phn-car-model-list-v2 .car-model-list-item.sc-phn-car-model-list-v2:last-child{margin-bottom:0}";

const CarModelListV2 = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.series = EMPTY_SERIES;
    }
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "car-model-list" }, this.series.Models.map((model) => (_commonjsHelpers.h("phn-p-link", { variant: "tertiary", href: model.Link, icon: 'arrow-head-right', size: "inherit", class: "car-model-list-item", onClick: () => this.sendModelClickAnalyticsEvent(model) }, model.Name)))));
    }
};
CarModelListV2.style = carModelListV2Css;

/* istanbul ignore file */
const getSrcPath = (image) => {
    const path = _commonjsHelpers.getAssetPath(`${config(navContentStore.get('env')).ASSETS_URL}/series-assets`);
    const imageFileEnding = 'jpg';
...
const carSeriesCss = ".car-series.sc-phn-car-series-h{text-align:center;text-decoration:none;color:#000;display:block}.car-series.sc-phn-car-series-h:hover{cursor:pointer;color:#d5001c}@media (min-width: 0) and (max-width: 759px){.car-series.sc-phn-car-series-h{padding-bottom:0.5625rem}}@media (min-width: 760px) and (max-width: 999px){.car-series.sc-phn-car-series-h{padding-bottom:0.625rem}}@media (min-width: 1000px) and (max-width: 1299px){.car-series.sc-phn-car-series-h{padding-bottom:0.25rem}.car-series.sc-phn-car-series-h:focus{outline-width:2px;outline-style:solid;outline-offset:1px;outline-color:currentColor;outline-offset:-2px}}@media (min-width: 1300px) and (max-width: 1759px){.car-series.sc-phn-car-series-h{height:5.5625rem;min-width:5.5625rem}.car-series.sc-phn-car-series-h:hover{color:inherit;cursor:default}}@media (min-width: 1760px){.car-series.sc-phn-car-series-h{height:100px}.car-series.sc-phn-car-series-h:hover{color:inherit;cursor:default}}.car-series.sc-phn-car-series-h .car-series-text.sc-phn-car-series{padding:0;margin:0;line-height:1.5rem;font-size:1rem}@media (min-width: 0) and (max-width: 759px){.car-series.sc-phn-car-series-h .car-series-text.sc-phn-car-series{margin:-1.125rem 0 0 0;padding-right:0.1875rem}}@media (min-width: 760px) and (max-width: 999px){.car-series.sc-phn-car-series-h .car-series-text.sc-phn-car-series{margin:-2.3125rem 0 0 0;padding-right:0.0625rem;font-size:1.125rem}}@media (min-width: 1000px) and (max-width: 1299px){.car-series.sc-phn-car-series-h .car-series-text.sc-phn-car-series{margin:-2.0625rem 0 0 0;padding-right:0.125rem;font-size:1rem}}@media (min-width: 1300px) and (max-width: 1759px){.car-series.sc-phn-car-series-h .car-series-text.sc-phn-car-series{margin-top:-1.0625rem;font-size:0.875rem;padding-bottom:1.25rem}}@media (min-width: 1760px){.car-series.sc-phn-car-series-h .car-series-text.sc-phn-car-series{margin-top:-1.1875rem;padding-bottom:1.4375rem}}.car-series-skeleton.sc-phn-car-series{pointer-events:none;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;background:#e3e4e5;pointer-events:none;display:none}@media (min-width: 1300px){.car-series-skeleton.sc-phn-car-series{display:block;margin-top:1rem;height:2.6875rem;width:5.5625rem}}@media (min-width: 1760px){.car-series-skeleton.sc-phn-car-series{margin-top:1.1875rem;height:2.6875rem;width:6.5625rem}}";

const CarSeries = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    ...
    render() {
        ...
        return (_commonjsHelpers.h(_commonjsHelpers.Host, { class: "car-series" }, _commonjsHelpers.h("p", { style: navStateStore.get('navigationLoaded') ? {} : hiddenStyle, class: "car-series-text", "aria-label": `Porsche Model Series ${this.text}` }, this.text), _commonjsHelpers.h("div", { style: navStateStore.get('navigationLoaded') ? hiddenStyle : {}, class: "car-series-skeleton" })));
    }
...
const carSeriesListCss = ".sc-phn-car-series-list-h{display:-ms-flexbox;display:flex;-ms-flex-wrap:wrap;flex-wrap:wrap;margin-top:1.75rem}@media (min-width: 760px) and (max-width: 1299px){.sc-phn-car-series-list-h{margin-top:1.875rem}}@media (min-width: 1300px){.sc-phn-car-series-list-h{-ms-flex-wrap:nowrap;flex-wrap:nowrap;margin-top:0}}@media (min-width: 1300px) and (max-width: 1759px){.sc-phn-car-series-list-h{min-width:657px;margin-left:0.5625rem}}@media (min-width: 1760px){.sc-phn-car-series-list-h{min-width:809px;margin-left:1.25rem}}@media (min-width: 1300px) and (max-width: 1759px){.sc-phn-car-series-list-h .car-series-fly-out-overlay.sc-phn-car-series-list{padding:0 0.3125rem}.sc-phn-car-series-list-h .car-series-fly-out-overlay.sc-phn-car-series-list:first-of-type{padding-left:0.9375rem}.sc-phn-car-series-list-h .car-series-fly-out-overlay.sc-phn-car-series-list:last-of-type{padding-right:0.9375rem}}@media (min-width: 1760px){.sc-phn-car-series-list-h .car-series-fly-out-overlay.sc-phn-car-series-list{padding:0 0.5625rem}.sc-phn-car-series-list-h .car-series-fly-out-overlay.sc-phn-car-series-list:first-of-type{padding-left:1.25rem}.sc-phn-car-series-list-h .car-series-fly-out-overlay.sc-phn-car-series-list:last-of-type{padding-right:1.25rem}}";

const CarSeriesList = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    }
    render() {
        const carSeries = navContentStore.get('seriesList');
        return (_commonjsHelpers.h(_commonjsHelpers.Host, { role: "list" }, isFeatureEnabled(FEATURE_NEW_LEVEL2)
            ? carSeries.map((series) => (_commonjsHelpers.h("phn-mega-fly-out-overlay", { class: `car-series-fly-out-overlay overlay-trigger ${series.PhraseId}`, showOverlayHandler: this.showOverlayHandler, hideOverlayHandler: this.hideOverlayHandler, keyboardShowOverlayHandler: this.keyboardShowOverlayHandler, keyboardHideOverlayHandler: this.keyboardHideOverlayHandler, screenReaderId: `car-series-mega-menu-${series.Name}`, role: "listitem" }, _commonjsHelpers.h("phn-car-series", { slot: "button", text: series.Name, image: series.Image }), _commonjsHelpers.h("phn-car-series-model-overview-v2", { slot: "content", series: series, isMobile: false }))))
            : carSeries.map((series) => (_commonjsHelpers.h("phn-mega-fly-out-overlay", { class: `car-series-fly-out-overlay overlay-trigger ${series.PhraseId}`, showOverlayHandler: this.showOverlayHandler, hideOverlayHandler: this.hideOverlayHandler, keyboardShowOverlayHandler: this.keyboardShowOverlayHandler, keyboardHideOverlayHandler: this.keyboardHideOverlayHandler, screenReaderId: `car-series-mega-menu-${series.Name}`, role: "listitem" }, _commonjsHelpers.h("phn-car-series", { slot: "button", text: series.Name, image: series.Image }), _commonjsHelpers.h("phn-car-series-model-overview", { slot: "content", series: series }))))));
    }
};
CarSeriesList.style = carSeriesListCss;

class ModelRangeOverlayLoadEvent extends ModelAnalyticsEvent {
    constructor(analyticsSetup, modelRangeName, series) {
        super(analyticsSetup, modelRangeName);
        this.context.eventAction = 'PAGHomeNav_ModelRangeOverlay_Load';
        this.populateComponentClick(series);
    }
    populateComponentClick(series) {
...
const CarSeriesModelOverview = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.series = EMPTY_SERIES;
    }
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "car-series-model-overview" }, _commonjsHelpers.h("phn-car-meta-functionalities", { name: this.series.Name, items: this.series.Meta, modelMetaItems: this.series.ModelMeta }), _commonjsHelpers.h("phn-car-model-list", { series: this.series }), _commonjsHelpers.h("phn-car-further-links", { items: this.series.Further })));
    }
};

const carSeriesModelOverviewV2Css = ".series-headline.sc-phn-car-series-model-overview-v2{border-bottom:1px solid #0e1418}@media (min-width: 0) and (max-width: 759px){.series-headline.sc-phn-car-series-model-overview-v2{padding-bottom:0.5625rem}}@media (min-width: 760px) and (max-width: 999px){.series-headline.sc-phn-car-series-model-overview-v2{padding-bottom:0.8125rem}}@media (min-width: 1000px) and (max-width: 1299px){.series-headline.sc-phn-car-series-model-overview-v2{padding-bottom:0.875rem}}.limit-stretch.sc-phn-car-series-model-overview-v2{width:calc(100vw - 7vw * 2);max-width:96rem;margin-left:auto;margin-right:auto;-webkit-box-sizing:content-box;box-sizing:content-box}@media (min-width: 1760px){.limit-stretch.sc-phn-car-series-model-overview-v2{width:calc(100vw - 10vw * 2)}}@media (min-width: 1000px){.limit-stretch.sc-phn-car-series-model-overview-v2{padding-left:calc(100vw - 100%)}}";

const CarSeriesModelOverviewV2 = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.isMobile = true;
        this.series = EMPTY_SERIES;
    ...
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("div", { class: this.isDesktop ? 'limit-stretch' : '' }, _commonjsHelpers.h("phn-p-grid", { class: "car-series-model-overview" }, _commonjsHelpers.h("phn-p-grid-item", { size: "{base: 12, l:6}" }, _commonjsHelpers.h("phn-car-frontshot", { text: this.series.Name, image: this.series.Image })), this.isMobile ? (_commonjsHelpers.h("phn-p-grid-item", { size: "12" }, _commonjsHelpers.h("phn-p-text", { class: "series-headline", size: "large", weight: "semibold" }, this.series.Name))) : null, _commonjsHelpers.h("phn-p-grid-item", { size: "{base: 12, l:3}" }, _commonjsHelpers.h("phn-car-meta-functionalities-v2", { name: this.series.Name, items: this.series.Meta, modelMetaItems: this.series.ModelMeta })), _commonjsHelpers.h("phn-p-grid-item", { size: "{base: 12, l:3}" }, _commonjsHelpers.h("phn-car-model-list-v2", { series: this.series })))), navContentStore.get('furtherLinks') ? _commonjsHelpers.h("phn-car-further-links-v2", null) : null));
    }
};
CarSeriesModelOverviewV2.style = carSeriesModelOverviewV2Css;

class CrestClickEvent extends AnalyticsEvent {
    constructor(analyticsSetup) {
        super(analyticsSetup);
        this.context.eventAction = 'PAGHomeNav_Crest_Click';
    }
    event() {
        return super.event();
    }
...
const Crest = class {
    ...
    render() {
        const hiddenStyle = { display: 'none' };
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("a", { class: "crest", style: this.contentLoaded() ? {} : hiddenStyle, onClick: () => this.sendCrestClickAnalyticsEvent(), href: this.getCrestLink() }, _commonjsHelpers.h("phn-p-marque", { class: "marque" })), _commonjsHelpers.h("div", { class: "marque-bone", style: this.contentLoaded() ? hiddenStyle : {} }, _commonjsHelpers.h("span", { class: "crest-bone" }), _commonjsHelpers.h("span", { class: "label-bone" }))));
    }
    get el() { return _commonjsHelpers.getElement(this); }
};
Crest.style = crestCss;

const dealerSearchCss = ".dealer-search.sc-phn-dealer-search-h .dealer-search-link.sc-phn-dealer-search{border-bottom:1px solid #e3e4e5;padding:0.875rem 0 0.875rem 0;width:100%}@media (min-width: 760px) and (max-width: 999px){.dealer-search.sc-phn-dealer-search-h .dealer-search-link.sc-phn-dealer-search{padding:1.125rem 0 1.125rem 0}}@media (min-width: 1300px){.dealer-search.sc-phn-dealer-search-h .dealer-search-link.sc-phn-dealer-search{border-bottom:0;padding:0}}@media (min-width: 1300px){.dealer-search.sc-phn-dealer-search-h .dealer-search-skeleton.sc-phn-dealer-search{width:6.375rem;height:0.875rem;background-color:#e3e4e5;margin-top:0.125rem;width:8rem}}";

const DealerSearch = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    ...
    render() {
        const hiddenStyle = { display: 'none' };
        return (_commonjsHelpers.h(_commonjsHelpers.Host, { class: "dealer-search" }, _commonjsHelpers.h("phn-p-link-pure", { style: navStateStore.get('navigationLoaded') ? {} : hiddenStyle, href: this.dealerSearchLink, size: "inherit", icon: "pin", class: 'dealer-search-link', onClick: () => {
                const linkItem = {
                    PhraseId: DEALER_SEARCH_PHRASE_ID,
                    Text: this.dealerSearchText,
                    Link: this.dealerSearchLink
                };
                this.sendMetaFunctionClickAnalyticsEvent(linkItem);
            } }, this.dealerSearchText), _commonjsHelpers.h("div", { class: "dealer-search-skeleton", style: navStateStore.get('navigationLoaded') ? hiddenStyle : {} })));
    }
    get el() { return _commonjsHelpers.getElement(this); }
};
DealerSearch.style = dealerSearchCss;

const desktopShopAndMoreItemCss = ".desktop-shop-and-more-item.sc-phn-desktop-shop-and-more-item-h{display:-ms-flexbox;display:flex;color:#000;font-size:1rem;border-bottom:0;padding:1.3125rem 0 2.75rem 0;white-space:nowrap}.desktop-shop-and-more-item.sc-phn-desktop-shop-and-more-item-h:hover{cursor:default;color:inherit}@media (min-width: 1760px){.desktop-shop-and-more-item.sc-phn-desktop-shop-and-more-item-h{padding:1.6875rem 0 2.8125rem 0;font-size:1.25rem}}.desktop-shop-and-more-item-skeleton.sc-phn-desktop-shop-and-more-item{pointer-events:none;-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;display:-ms-flexbox;display:flex;background:#e3e4e5;-ms-flex-align:center;align-items:center;height:0.875rem}@media (min-width: 1300px){.desktop-shop-and-more-item-skeleton.sc-phn-desktop-shop-and-more-item{width:2.375rem;margin-top:0.375rem}}@media (min-width: 1760px){.desktop-shop-and-more-item-skeleton.sc-phn-desktop-shop-and-more-item{width:3.125rem;margin-top:0.1875rem}}";

const DesktopShopAndMoreItem = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    }
    render() {
        const hiddenStyle = { display: 'none' };
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("phn-p-text", { style: navStateStore.get('navigationLoaded') ? {} : hiddenStyle, class: "desktop-shop-and-more-item-text", color: "inherit", size: "inherit", tag: "div" }, this.itemText), _commonjsHelpers.h("div", { class: "desktop-shop-and-more-item-skeleton", style: navStateStore.get('navigationLoaded') ? hiddenStyle : {} })));
    }
};
DesktopShopAndMoreItem.style = desktopShopAndMoreItemCss;

const DrawerCarSeriesModelOverview = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.series = EMPTY_SERIES;
    }
    render() {
        return (_commonjsHelpers.h("div", { class: "drawer-car-series-model-overview" }, _commonjsHelpers.h("div", { class: "drawer-container drawer-level-2-item-scroll" }, _commonjsHelpers.h("phn-headline", { text: this.series.Name }), _commonjsHelpers.h("phn-car-meta-functionalities", { name: this.series.Name, items: this.series.Meta, modelMetaItems: this.series.ModelMeta }), _commonjsHelpers.h("phn-car-model-list", { class: "car-model-list", series: this.series }), _commonjsHelpers.h("phn-car-further-links", { items: this.series.Further }))));
    }
};

const drawerShopsAndMoreCss = ".drawer-shops-and-more.sc-phn-drawer-shops-and-more{padding-bottom:3.125rem}";

const DrawerShopsAndMore = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.item = EMPTY_ITEM;
    }
    showDrawer(item, index) {
        const phnDrawerShopsAndMore = {
            backButton: this.item.Text,
            content: (_commonjsHelpers.h("phn-drawer-shops-and-more", { headline: item.Text, item: item, class: 'shops-and-more-drawer-level-2 drawer-level-2 drawer-level-2-' + index, slot: "content" }))
        };
        if (!item.Link && item.Items) {
            navStateStore.set('drawerContentStack', [...navStateStore.get('drawerContentStack'), phnDrawerShopsAndMore]);
        }
    }
    render() {
        return (_commonjsHelpers.h("div", { class: `drawer-shops-and-more` }, _commonjsHelpers.h("div", { class: "drawer-container" }, _commonjsHelpers.h("phn-headline", { text: this.headline, class: "drawer-headline" }), this.item.Items &&
            this.item.Items.map((item, index) => {
                return (_commonjsHelpers.h("div", null, _commonjsHelpers.h("phn-list-item", { item: item, onClick: () => this.showDrawer(item, index), class: 'drawer-shops-and-more-item drawer-shops-and-more-item-' + index })));
            }))));
    }
};
...
const infoLogRequest = (id, data, env) => {
    logRequest(env, 'info', id, data);
};
const debugLogRequest = (id, data, env) => {
    logRequest(env, 'debug', id, data);
};
const errorLogRequest = (id, data, env) => {
    logRequest(env, 'error', id, data);
};

const fetchContent = async (env) => {
    const loadData = async () => {
        ...
        try {
            ...
            errorLogRequest('content', `Failed to fetch content with error: ${e}`, env);
...
const getContentForLocale = (locale, content, env) => {
...
const updateLoggedInStatus = async (newToken, localeObject) => {
...
const updateUnreadMessagesCount = async (accessToken) => {
    const env = navContentStore.get('env');
...
var xhr = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var authorization_service_configuration = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var crypto_utils = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var flags = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var authorization_request = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var AuthorizationRequest = /** @class */ (function () {
    ...
    function AuthorizationRequest(request, crypto, usePkce) {
        if (crypto === void 0) { crypto = new crypto_utils.DefaultCrypto(); }
...
var authorization_request_handler = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var authorization_response = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var query_string_utils = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var storage = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var redirect_based_handler = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var RedirectRequestHandler = /** @class */ (function (_super) {
    ...
    storageBackend, utils, locationLike, crypto) {
        if (storageBackend === void 0) { storageBackend = new storage.LocalStorageBackend(); }
        if (utils === void 0) { utils = new query_string_utils.BasicQueryStringUtils(); }
...
var token_response = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var token_request_handler = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
var BaseTokenRequestHandler = /** @class */ (function () {
    function BaseTokenRequestHandler(requestor, utils) {
        if (requestor === void 0) { requestor = new xhr.JQueryRequestor(); }
        if (utils === void 0) { utils = new query_string_utils.BasicQueryStringUtils(); }
...
var token_request = _commonjsHelpers.createCommonjsModule(function (module, exports) {
/*
 * Copyright 2017 Google Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file except
...
class NoHashQueryStringUtils extends query_string_utils.BasicQueryStringUtils {
    constructor(code, state, error) {
        super();
        this.code = code;
        this.state = state;
        this.error = error;
    }
    parse(_query, _useHash) {
        return {
            code: this.code,
            state: this.state,
            error: this.error
...
const fetchAccessToken = async (tokenReceivedHandler, errorHandler, env) => {
    let authorizationServiceConfiguration, tokenHandler, authorizationHandler;
    const iFrameAuth = createHiddenIFrame();
    const envConfig = config(env);
    const fetchRequestor = new xhr.FetchRequestor();
    try {
        authorizationServiceConfiguration = await authorization_service_configuration.AuthorizationServiceConfiguration.fetchFromIssuer(envConfig.LOGIN_BASE_URL, fetchRequestor);
        ...
        const getToken = async (authorizationRequestResponse) => {
            ...
            const requestToken = new token_request.TokenRequest({
            ...
            try {
                ...
                errorLogRequest('silent_auth', `Token request failed: ${JSON.stringify(e.message)}`, env);
                errorHandler();
            }
        };
        const request = new authorization_request.AuthorizationRequest({
            client_id: envConfig.CLIENT_ID,
            redirect_uri: envConfig.LOGIN_REDIRECT_URL,
            scope: 'openid',
            ...
            if (envConfig.LOGIN_REDIRECT_URL.indexOf(event.origin) > -1) {
                if (event.data.state && event.data.code) {
                    ...
                    try {
                        ...
                        errorLogRequest('silent_auth', `Something really bad happened: ${JSON.stringify(e.message)}`, env);
                        errorHandler();
                    }
                }
        ...
        errorLogRequest('silent_auth', `Something bad happened: ${error.message}`, env);
        errorHandler();
    }
};
const emitAccessToken = (element, env) => {
...
class GeneralLoadEvent extends AnalyticsEvent {
    constructor(analyticsSetup) {
        super(analyticsSetup);
        this.context.eventAction = 'PAGHomeNav_General_Load';
    }
    event() {
        return super.event();
    }
...
const Header = class {
    constructor(hostRef) {
        ...
        this.addFocus = () => {
            const skipContentElement = this.el.shadowRoot.querySelector('phn-p-button.skip-content');
    ...
    localeWatchHandler(newLocale) {
        ...
        this.accessibilityStatement = navContentStore.get('accessibilityStatement');
    ...
    accessTokenStateUpdated(newToken, oldToken) {
        if (newToken) {
            if (newToken !== oldToken) {
                updateLoggedInStatus(newToken, this.localeObject).then((statusUpdated) => {
                    if (!statusUpdated) {
    ...
    async componentWillLoad() {
        ...
        this.content = await fetchContent(this.env);
        const contentForLocale = getContentForLocale(this.localeObject, this.content, this.env);
        ...
        if (this.accessToken) {
            ...
            navContentStore.set('loggedInState', LOADING_STATUS);
            await emitAccessToken(this.el, this.env)();
        }
        else {
            navContentStore.set('loggedInState', LOGGED_OUT_USER_STATUS);
        }
        navContentStore.set('accessibilityStatement', contentForLocale.accessibilityStatement);
        this.accessibilityStatement = navContentStore.get('accessibilityStatement');
        navContentStore.set('furtherLinks', contentForLocale.furtherLinks);
    ...
    render() {
        return (_commonjsHelpers.h("header", { class: "header-container" }, isFeatureEnabled(FEATURE_ACCESSIBILITY_STATEMENT) && this.locale == 'en-US' ? (_commonjsHelpers.h("a", { class: "sr-only", href: this.accessibilityStatement.Link }, this.accessibilityStatement.Text)) : null, _commonjsHelpers.h("phn-p-button", { variant: "primary", class: "skip-content", onfocus: this.addFocus, onClick: this.skipToMainContent, onblur: this.removeFocus }, "Skip to main content"), _commonjsHelpers.h("div", { class: "header limit-stretch" }, _commonjsHelpers.h("div", { class: "crest-burger-btn-container" }, _commonjsHelpers.h("phn-crest", { class: "crest" }), _commonjsHelpers.h("phn-mobile-layout", null)), _commonjsHelpers.h("nav", { class: "menu-container" }, _commonjsHelpers.h("phn-menu-desktop-level-1", { class: "menu-desktop-level-1" }))), _commonjsHelpers.h("div", { class: "header-border" }), this.env === 'preview' && _commonjsHelpers.h("phn-preview-badge", { versionString: `Header ${VERSION} - PP content` }), _commonjsHelpers.h("div", { id: "main", tabindex: "-1", class: "main-content", onBlur: this.mainContentTab })));
    }
...
const Headline = class {
    ...
    render() {
        return _commonjsHelpers.h("h1", { class: "headline" }, this.text);
    }
};
Headline.style = headlineCss;

const listItemCss = ".list-item-link.sc-phn-list-item{-webkit-box-sizing:border-box;box-sizing:border-box;font-size:1rem;border-bottom:1px solid #e3e4e5}.list-item-link.sc-phn-list-item:hover{color:#d5001c;cursor:pointer}@media (min-width: 760px) and (max-width: 999px){.list-item-link.sc-phn-list-item{font-size:1.125rem}}@media (min-width: 1300px){.list-item-link.sc-phn-list-item{border-bottom:0;padding-left:0}}.list-item-link.sc-phn-list-item a.sc-phn-list-item{font-size:1rem;line-height:1.5;font-family:\"Porsche Next\", \"Arial Narrow\", Arial, sans-serif;font-weight:400;color:inherit;text-decoration:none;line-height:1.5rem;display:inline-block;width:100%;padding:0.875rem 0 0.875rem 0.625rem}.list-item-link.sc-phn-list-item a.sc-phn-list-item::-moz-focus-inner{border:0}.list-item-link.sc-phn-list-item a.sc-phn-list-item{outline:transparent solid 1px;outline-offset:2px}.list-item-link.sc-phn-list-item a.sc-phn-list-item:focus{outline-color:currentColor}.list-item-link.sc-phn-list-item a.sc-phn-list-item:focus:not(:focus-visible){outline-color:transparent}@media (min-width: 760px) and (max-width: 999px){.list-item-link.sc-phn-list-item a.sc-phn-list-item{padding-top:1.125rem;padding-bottom:1.125rem}}@media (min-width: 1300px){.list-item-link.sc-phn-list-item a.sc-phn-list-item{padding-bottom:0}}.list-item-button.sc-phn-list-item{-webkit-box-sizing:border-box;box-sizing:border-box;font-size:1rem;border-bottom:1px solid #e3e4e5;height:3.3125rem;padding-left:0.625rem;display:-ms-flexbox;display:flex;-ms-flex-align:center;align-items:center}@media (min-width: 760px) and (max-width: 999px){.list-item-button.sc-phn-list-item{font-size:1.125rem;height:3.8125rem;padding-right:0.625rem}}@media (min-width: 1300px){.list-item-button.sc-phn-list-item{border-bottom:0;padding-left:0;padding-top:0.875rem}}.list-item-button.sc-phn-list-item .list-item-text.sc-phn-list-item{display:inline-block;line-height:24px}";
...
const ListItem = class {
    ...
    renderLinkItem(linkItem) {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("div", { class: "list-item-link" }, _commonjsHelpers.h("a", { href: linkItem.Link, onClick: () => this.sendNavigationClickAnalyticsEvent(linkItem) }, linkItem.Text))));
    }
    renderTextItem(textItem) {
        return (_commonjsHelpers.h("phn-button-extended", { class: "list-item-button", icon: "arrow-head-right", onClick: () => this.sendNavigationClickAnalyticsEvent(textItem), cancelTabIndex: false }, _commonjsHelpers.h("span", { class: "list-item-text" }, textItem.Text)));
    }
    render() {
...
const megaFlyOutOverlayCss = ".backdrop.sc-phn-mega-fly-out-overlay{position:fixed;z-index:200;left:0;right:0;background:rgba(0, 0, 0, 0.5);pointer-events:none;height:100vh;margin-top:1px}@media (min-width: 1300px){.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-button.sc-phn-mega-fly-out-overlay{display:-ms-flexbox;display:flex;font-family:inherit;width:100%;margin:0;padding:0;-webkit-box-sizing:border-box;box-sizing:border-box;outline:transparent none;-webkit-appearance:none;-moz-appearance:none;appearance:none;border:none;text-decoration:none;text-align:left;background:transparent;cursor:pointer;color:inherit;cursor:default;border-bottom:2px solid transparent}.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-button.sc-phn-mega-fly-out-overlay::-moz-focus-inner{border:0}.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-button.sc-phn-mega-fly-out-overlay:focus{outline-width:2px;outline-style:solid;outline-offset:1px;outline-color:currentColor}.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-button-active.sc-phn-mega-fly-out-overlay{border-bottom:2px solid #d5001c}.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-button-active.sc-phn-mega-fly-out-overlay:hover{color:#d5001c}}.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-container.sc-phn-mega-fly-out-overlay{display:block;visibility:hidden;width:100%;position:absolute;left:0;z-index:300;max-height:0;overflow:hidden}@media (min-width: 1300px) and (max-width: 1759px){.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-container.sc-phn-mega-fly-out-overlay{top:117px}}@media (min-width: 1760px){.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-container.sc-phn-mega-fly-out-overlay{top:123px}}.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-container.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-content.sc-phn-mega-fly-out-overlay{background-color:#fff;border-top:1px solid #e3e4e5}@media (min-width: 1300px) and (max-width: 1759px){.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-container.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-content.sc-phn-mega-fly-out-overlay{padding-bottom:5rem}}@media (min-width: 1760px){.mega-fly-out-overlay.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-container.sc-phn-mega-fly-out-overlay .mega-fly-out-overlay-content.sc-phn-mega-fly-out-overlay{padding-bottom:6rem}}";

const MegaFlyOutOverlay = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.active = '';
    ...
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("div", { class: "mega-fly-out-overlay" }, _commonjsHelpers.h("button", { class: 'mega-fly-out-overlay-button mega-fly-out-overlay-button-' + this.active, onKeyDown: this.handleKeyDown, onClick: this.handleClick, "aria-expanded": this.active === 'active' ? 'true' : 'false', "aria-haspopup": "true", id: this.screenReaderId }, _commonjsHelpers.h("slot", { name: "button" })), _commonjsHelpers.h("div", { class: "mega-fly-out-overlay-container", ref: (el) => (this.containerElement = el), onKeyDown: this.handleKeyDownContainer, "aria-labelledby": this.screenReaderId, "aria-hidden": this.active === 'active' ? 'false' : 'true', role: "region" }, _commonjsHelpers.h("div", { class: "mega-fly-out-overlay-content" }, _commonjsHelpers.h("slot", { name: "content" }))), this.active && _commonjsHelpers.h("div", { class: "backdrop", ref: (el) => (this.backdropElement = el) }))));
    }
    get element() { return _commonjsHelpers.getElement(this); }
};
MegaFlyOutOverlay.style = megaFlyOutOverlayCss;

class HoverIntentService {
    constructor() {
        this.last = null;
    }
...
class OverlayAnimation {
    constructor() {
        ...
        this.slideDown = (element) => {
            ...
            this.showOverlay = gsapWithCSS.to(element, {
                maxHeight: '1000px',
                duration: 0.54,
                ease: 'sine.inOut',
                visibility: 'visible'
        ...
        this.show = (element) => {
            this.killAnimation(this.showOverlay);
            this.killAnimation(this.hideOverlay);
            this.showOverlay = gsapWithCSS.set(element, {
                maxHeight: '1000px',
                visibility: 'visible'
            });
        };
        this.hide = (element) => {
            this.killAnimation(this.showOverlay);
            this.killAnimation(this.hideOverlay);
            this.hideOverlay = gsapWithCSS.set(element, {
                maxHeight: '0',
                onComplete: () => {
        ...
        this.slideUp = (element) => {
            this.killAnimation(this.showOverlay);
            this.killAnimation(this.hideOverlay);
            this.hideOverlay = gsapWithCSS.to(element, {
                maxHeight: '0',
                duration: 0.43,
                ease: 'sine.inOut',
                onComplete: () => {
...
const menuDesktopLevel1Css = ".menu-desktop-level-1.sc-phn-menu-desktop-level-1{display:-ms-flexbox;display:flex;-ms-flex:1;flex:1;-ms-flex-direction:row;flex-direction:row;-ms-flex-pack:justify;justify-content:space-between}@media (min-width: 1300px){.menu-desktop-level-1.sc-phn-menu-desktop-level-1{padding-top:1.625rem}}@media (min-width: 1760px){.menu-desktop-level-1.sc-phn-menu-desktop-level-1{padding-top:1.3125rem}}.menu-desktop-level-1.sc-phn-menu-desktop-level-1 .menu-series-shop-and-more.sc-phn-menu-desktop-level-1{display:-ms-inline-flexbox;display:inline-flex}.menu-desktop-level-1.sc-phn-menu-desktop-level-1 .menu-separator.sc-phn-menu-desktop-level-1{width:1px;height:2.375rem;background-color:#e3e4e5;margin-top:0.875rem}@media (min-width: 1760px){.menu-desktop-level-1.sc-phn-menu-desktop-level-1 .menu-separator.sc-phn-menu-desktop-level-1{height:3rem;margin-top:1.0625rem}}.menu-desktop-level-1.sc-phn-menu-desktop-level-1 .metafunctionalities.sc-phn-menu-desktop-level-1{padding-top:1.3125rem}@media (-ms-high-contrast: none) and (min-width: 1300px) and (max-width: 1759px){.menu-desktop-level-1.sc-phn-menu-desktop-level-1 .metafunctionalities.sc-phn-menu-desktop-level-1{-ms-flex:1;flex:1}}@media (min-width: 1300px) and (max-width: 1759px){.menu-desktop-level-1.sc-phn-menu-desktop-level-1 .metafunctionalities.sc-phn-menu-desktop-level-1{padding-top:0}}@media (min-width: 1760px){.menu-desktop-level-1.sc-phn-menu-desktop-level-1 .metafunctionalities.sc-phn-menu-desktop-level-1{padding-top:1.6875rem}}";

const MenuDesktopLevel1 = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.overlayEvent = _commonjsHelpers.createEvent(this, "overlayEvent", 7);
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "menu-desktop-level-1" }, _commonjsHelpers.h("div", { class: "menu-series-shop-and-more", onMouseEnter: () => this.handleMouseEnter(), onMouseLeave: () => this.handleMouseLeave() }, _commonjsHelpers.h("phn-car-series-list", { showOverlayHandler: this.showOverlayHandler, hideOverlayHandler: this.hideOverlayHandler, keyboardShowOverlayHandler: this.keyboardShowOverlayHandler, keyboardHideOverlayHandler: this.keyboardHideOverlayHandler }), _commonjsHelpers.h("phn-shops-and-more", { showOverlayHandler: this.showOverlayHandler, hideOverlayHandler: this.hideOverlayHandler, keyboardShowOverlayHandler: this.keyboardShowOverlayHandler, keyboardHideOverlayHandler: this.keyboardHideOverlayHandler })), navStateStore.get('navigationLoaded') ? _commonjsHelpers.h("div", { class: "menu-separator" }) : null, _commonjsHelpers.h("phn-meta-functionalities", { class: "metafunctionalities" })));
    }
};
MenuDesktopLevel1.style = menuDesktopLevel1Css;

class MenuDrawerAnimation {
    constructor() {
...
const MenuDrawer = class {
    ...
    render() {
        ...
        return (_commonjsHelpers.h(_commonjsHelpers.Host, { class: "drawer-trigger", tabindex: "-1" }, _commonjsHelpers.h("div", { class: "menu-drawer" }, _commonjsHelpers.h("div", { class: "menu-drawer-container", onClick: this.close }, _commonjsHelpers.h("div", { class: "menu-drawer-content", onClick: this.stopClickEvent }, _commonjsHelpers.h("div", { class: "menu-drawer-content-wrapper", role: "dialog", "aria-modal": "true" }, _commonjsHelpers.h("span", { ref: (el) => (this.firstKeyboardAnchor = el), "aria-hidden": "true", tabindex: "-1" }), _commonjsHelpers.h("div", { class: backAndCloseClass }, !this.isFirstDrawer && (_commonjsHelpers.h("phn-back-button", { class: "back-button", onClick: this.handleBack, buttonId: backButtonId, text: backButtonText })), _commonjsHelpers.h("phn-p-button-pure", { class: "close-button", icon: "close", hideLabel: "true", "aria-label": "close dialog", size: "small", onClick: this.close }, "Close Dialog")), _commonjsHelpers.h("slot", { name: "content" }), _commonjsHelpers.h("span", { ref: (el) => (this.lastKeyboardAnchor = el), "aria-hidden": "true", tabindex: "-1" })))))));
    }
    get el() { return _commonjsHelpers.getElement(this); }
};
MenuDrawer.style = menuDrawerCss;

const MenuMobileLevel1 = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    }
    render() {
        const seriesList = navContentStore.get('seriesList');
        return (_commonjsHelpers.h("div", { class: "menu-mobile-level-1 mobile-level-1-item" }, _commonjsHelpers.h("div", { class: "mobile-level-1 mobile-level-1-item-scroll" }, _commonjsHelpers.h("div", { class: "drawer-container" }, _commonjsHelpers.h("div", { class: "drawer-content" }, _commonjsHelpers.h("phn-mobile-car-series-list", { class: "mobile-car-series-list", carSeries: seriesList }), _commonjsHelpers.h("phn-mobile-shops-and-more", null), _commonjsHelpers.h("phn-meta-functionalities", { class: "meta-functionalities" }))))));
    }
};

const messagesLinkCss = ".sc-phn-messages-link-h .list-item-link.sc-phn-messages-link{display:block;-webkit-box-sizing:border-box;box-sizing:border-box;font-size:1rem;border-bottom:1px solid #e3e4e5}.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link:hover{color:#d5001c;cursor:pointer}@media (min-width: 760px) and (max-width: 999px){.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link{font-size:1.125rem}}@media (min-width: 1300px){.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link{border-bottom:0;padding-left:0}}.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link a.sc-phn-messages-link{text-decoration:none;display:-ms-inline-flexbox;display:inline-flex;-ms-flex-align:center;align-items:center;width:100%;padding:0.875rem 0 0.875rem 0.625rem}@media (min-width: 760px) and (max-width: 999px){.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link a.sc-phn-messages-link{padding-top:1.125rem;padding-bottom:1.125rem}}@media (min-width: 1300px){.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link a.sc-phn-messages-link{padding-bottom:0}}.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link .unread-messages-count.sc-phn-messages-link{height:1rem;margin-left:0.5rem;padding:0 0.3125rem;background-color:#d5001c;border-radius:8px;font-size:0.75rem;color:#fff;line-height:1rem}@media (min-width: 760px) and (max-width: 999px){.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link .unread-messages-count.sc-phn-messages-link{height:1.125rem;font-size:0.875rem;line-height:1.125rem}}.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link .unread-messages-count.sc-phn-messages-link:hover{color:#fff}.sc-phn-messages-link-h .list-item-link.sc-phn-messages-link .unread-messages-count.hidden.sc-phn-messages-link{display:none}";

const MessagesLink = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.item = EMPTY_ITEM;
    ...
    render() {
        ...
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("phn-p-text", { class: "list-item-link", tag: "div", color: "inherit", size: "inherit" }, _commonjsHelpers.h("a", { href: this.item.Link, onClick: this.sendNavigationClickAnalyticsEvent }, _commonjsHelpers.h("span", { class: "link-text" }, this.item.Text), _commonjsHelpers.h("span", { class: unreadMessagesCount > 0 ? 'unread-messages-count' : 'unread-messages-count hidden' }, unreadMessagesCount)))));
    }
};
MessagesLink.style = messagesLinkCss;

class MetaFunctionClick extends AnalyticsEvent {
    constructor(analyticsSetup, linkItem) {
        super(analyticsSetup);
        this.context.eventAction = 'PAGHomeNav_MetaFunction_Click';
        this.initComponentClick();
        this.populateComponentClick(linkItem);
    }
    initComponentClick() {
        this.componentClick = {
            [ComponentClickCollectionDimension.ClickElementId]: undefined,
    ...
    populateComponentClick(linkItem) {
...
const metaFunctionalitiesCss = ".metafunctionalities.sc-phn-meta-functionalities{display:-ms-flexbox;display:flex;-ms-flex-wrap:wrap;flex-wrap:wrap;-ms-flex-direction:column;flex-direction:column;font-size:1rem}@media (-ms-high-contrast: none) and (min-width: 1300px) and (max-width: 1759px){.metafunctionalities.sc-phn-meta-functionalities{float:right}}@media (min-width: 760px) and (max-width: 999px){.metafunctionalities.sc-phn-meta-functionalities{font-size:1.125rem}}@media (min-width: 1300px) and (max-width: 1759px){.metafunctionalities.sc-phn-meta-functionalities{-ms-flex-align:center;align-items:center;padding-top:0;height:4.0625rem;padding-left:1.25rem;-ms-flex-direction:row;flex-direction:row;font-size:1rem}.metafunctionalities.sc-phn-meta-functionalities .dealer-search.sc-phn-meta-functionalities{margin-right:1.125rem}}@media (min-width: 1760px){.metafunctionalities.sc-phn-meta-functionalities{-ms-flex-align:center;align-items:center;-ms-flex-direction:row;flex-direction:row;font-size:1.25rem}.metafunctionalities.sc-phn-meta-functionalities .dealer-search.sc-phn-meta-functionalities{margin-right:1.5rem}.metafunctionalities.sc-phn-meta-functionalities .dealer-search.sc-phn-meta-functionalities:only-child{margin-right:10.5625rem}}";

const MetaFunctionalities = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.sendMetaFunctionClickAnalyticsEvent = (linkItem) => {
            const analyticsSetup = {
    ...
    getFieldValueForPhraseId(phraseId, fieldName) {
        const functionality = navContentStore.get('metafunctionalities').find((meta) => meta.PhraseId === phraseId);
        if (!functionality) {
            return '';
        }
    ...
    myPorscheExists() {
        return (navContentStore.get('metafunctionalities').find((meta) => meta.PhraseId === MYPORSCHE_PHRASE_ID) !== undefined);
    }
    getLoggedInStatusTranslations() {
        const functionality = navContentStore
            .get('metafunctionalities')
            .find((meta) => meta.PhraseId === MYPORSCHE_PHRASE_ID);
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "metafunctionalities" }, _commonjsHelpers.h("phn-dealer-search", { class: "dealer-search", dealerSearchText: this.getFieldValueForPhraseId(DEALER_SEARCH_PHRASE_ID, TEXT_FIELD_NAME), dealerSearchLink: this.getFieldValueForPhraseId(DEALER_SEARCH_PHRASE_ID, LINK_FIELD_NAME), sendMetaFunctionClickAnalyticsEvent: this.sendMetaFunctionClickAnalyticsEvent }), this.myPorscheExists() && (_commonjsHelpers.h("phn-myporsche", { myporscheText: this.getFieldValueForPhraseId(MYPORSCHE_PHRASE_ID, TEXT_FIELD_NAME), myporscheLink: this.getFieldValueForPhraseId(MYPORSCHE_PHRASE_ID, LINK_FIELD_NAME), loggedInStatusTranslations: this.getLoggedInStatusTranslations(), sendMetaFunctionClickAnalyticsEvent: this.sendMetaFunctionClickAnalyticsEvent, setActiveDrawer: this.setActiveDrawer, closeActiveDrawer: this.closeActiveDrawer }))));
    }
    get hostElement() { return _commonjsHelpers.getElement(this); }
};
MetaFunctionalities.style = metaFunctionalitiesCss;

class ModelRangeClickEvent extends ModelAnalyticsEvent {
    constructor(analyticsSetup, modelRangeName, series) {
        super(analyticsSetup, modelRangeName);
        this.context.eventAction = 'PAGHomeNav_ModelRange_Click';
        this.populateComponentClick(series);
    }
    populateComponentClick(series) {
        this.componentClick.clickElementId = series.PhraseId;
...
const MobileCarSeriesList = class {
    ...
    showDrawer(_, series, index) {
        if (isFeatureEnabled(FEATURE_NEW_LEVEL2)) {
            const phnCarSeriesModelOverviewV2 = {
                content: (_commonjsHelpers.h("phn-car-series-model-overview-v2", { series: series, class: 'drawer-car-series-model-overview-' + index, slot: "content" }))
            };
            ...
            const phnDrawerCarSeriesModelOverview = {
                content: (_commonjsHelpers.h("phn-drawer-car-series-model-overview", { series: series, class: 'drawer-car-series-model-overview drawer-car-series-model-overview-' + index, slot: "content" }))
            };
            navStateStore.set('drawerContentStack', [
                ...navStateStore.get('drawerContentStack'),
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "mobile-car-series-list" }, this.carSeries.map((series, index) => (_commonjsHelpers.h("div", { class: "mobile-car-series", key: index }, _commonjsHelpers.h("button", { class: "car-series-button", onClick: (event) => this.showDrawer(event, series, index) }, _commonjsHelpers.h("phn-car-series", { class: 'car-series car-series-' + index, text: series.Name, image: series.Image })))))));
    }
};
...
const mobileLayoutCss = ".sc-phn-mobile-layout-h .burger-button.sc-phn-mobile-layout{height:100%}.sc-phn-mobile-layout-h .burger-button-skeleton.sc-phn-mobile-layout{height:0.875rem}";

const MobileLayout = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    ...
    showMobileLevel1() {
        const drawer = { content: _commonjsHelpers.h("phn-menu-mobile-level-1", { slot: "content" }) };
        navStateStore.set('drawerContentStack', [drawer]);
    }
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("phn-burger-button", { class: "burger-button", onClick: this.showMobileLevel1 }), navStateStore.get('drawerContentStack').map((drawer, index, array) => {
            const isTopmostDrawer = array.length - 1 === index;
            return (_commonjsHelpers.h("phn-menu-drawer", { key: index, backButton: drawer.backButton, mobileLayout: this.mobileLayoutElement, isTopmostDrawer: isTopmostDrawer ? 'true' : '' }, drawer.content));
        })));
    }
    get mobileLayoutElement() { return _commonjsHelpers.getElement(this); }
};
MobileLayout.style = mobileLayoutCss;

const mobileShopAndMoreItemCss = ".sc-phn-mobile-shop-and-more-item-h{display:-ms-flexbox;display:flex;color:#000;font-size:1rem;padding:0.875rem 0 1rem 0.625rem;border-bottom:1px solid #e3e4e5}.sc-phn-mobile-shop-and-more-item-h:hover{color:#d5001c;cursor:pointer}@media (min-width: 480px) and (max-width: 759px){.sc-phn-mobile-shop-and-more-item-h:focus{outline-width:2px;outline-style:solid;outline-offset:1px;outline-color:currentColor}}@media (min-width: 760px) and (max-width: 999px){.sc-phn-mobile-shop-and-more-item-h{padding:1.125rem 0 1.125rem 0.625rem;font-size:1.125rem}.sc-phn-mobile-shop-and-more-item-h:focus{outline-width:2px;outline-style:solid;outline-offset:1px;outline-color:currentColor}}@media (min-width: 1000px){.sc-phn-mobile-shop-and-more-item-h{padding:0.875rem 0 0.875rem 0.625rem}.sc-phn-mobile-shop-and-more-item-h:focus{outline-width:2px;outline-style:solid;outline-offset:1px;outline-color:currentColor}}.sc-phn-mobile-shop-and-more-item-h .mobile-shop-and-more-item-drawer.sc-phn-mobile-shop-and-more-item{display:-ms-flexbox;display:flex;width:100%}@media (min-width: 1300px){.sc-phn-mobile-shop-and-more-item-h .mobile-shop-and-more-item-drawer.sc-phn-mobile-shop-and-more-item{display:none}}";

const MobileShopAndMoreItem = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.item = EMPTY_ITEM;
    }
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, { class: "mobile-shop-and-more-item" }, _commonjsHelpers.h("phn-button-extended", { class: "mobile-shop-and-more-item-drawer", icon: "arrow-head-right" }, this.item.Text)));
    }
};
MobileShopAndMoreItem.style = mobileShopAndMoreItemCss;

const mobileShopsAndMoreCss = ".sc-phn-mobile-shops-and-more-h{display:-ms-flexbox;display:flex;-ms-flex-wrap:wrap;flex-wrap:wrap;-ms-flex-direction:column;flex-direction:column;border-top:1px solid #e3e4e5;margin-top:1.5rem}@media (min-width: 760px) and (max-width: 1299px){.sc-phn-mobile-shops-and-more-h{margin-top:3rem}}";

const MobileShopsAndMore = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.showDrawer = (_, item) => {
            const phnDrawerShopsAndMore = {
                content: (_commonjsHelpers.h("phn-drawer-shops-and-more", { headline: item.Text, item: item.Items && item.Items.length === 1 ? item.Items[0] : item, slot: "content", class: "shops-and-more-drawer-level-1" }))
            };
            navStateStore.set('drawerContentStack', [...navStateStore.get('drawerContentStack'), phnDrawerShopsAndMore]);
            this.sendNavigationClickAnalyticsEvent(item);
        };
    ...
    render() {
        ...
        return (_commonjsHelpers.h(_commonjsHelpers.Host, { class: "mobile-shops-and-more" }, items.map((item, index) => {
            return (_commonjsHelpers.h("div", { class: "shops-and-more-container" }, _commonjsHelpers.h("phn-mobile-shop-and-more-item", { item: item, class: 'mobile-shop-and-more-item mobile-shop-and-more-item-' + index, tabindex: "0", onClick: (event) => this.showDrawer(event, item), onKeyDown: (event) => this.handleKeyDown(event, item) })));
        })));
    }
};
MobileShopsAndMore.style = mobileShopsAndMoreCss;

const myporscheCss = ".myporsche.sc-phn-myporsche-h phn-p-link-pure.sc-phn-myporsche,.myporsche.sc-phn-myporsche-h phn-p-button-pure.sc-phn-myporsche{border-bottom:1px solid #e3e4e5;padding:0.875rem 0 0.875rem 0;width:100%}@media (min-width: 760px) and (max-width: 999px){.myporsche.sc-phn-myporsche-h phn-p-link-pure.sc-phn-myporsche,.myporsche.sc-phn-myporsche-h phn-p-button-pure.sc-phn-myporsche{padding:1.125rem 0 1.125rem 0}}@media (min-width: 1300px){.myporsche.sc-phn-myporsche-h phn-p-link-pure.sc-phn-myporsche,.myporsche.sc-phn-myporsche-h phn-p-button-pure.sc-phn-myporsche{border-bottom:0;padding:0}}.myporsche.sc-phn-myporsche-h phn-p-link-pure.sc-phn-myporsche .login-status.sc-phn-myporsche,.myporsche.sc-phn-myporsche-h phn-p-button-pure.sc-phn-myporsche .login-status.sc-phn-myporsche{margin-top:-0.25rem;font-size:0.75rem}@media (min-width: 760px) and (max-width: 999px){.myporsche.sc-phn-myporsche-h phn-p-link-pure.sc-phn-myporsche .login-status.sc-phn-myporsche,.myporsche.sc-phn-myporsche-h phn-p-button-pure.sc-phn-myporsche .login-status.sc-phn-myporsche{margin-top:-0.1875rem}}@media (min-width: 1300px){.myporsche.sc-phn-myporsche-h phn-p-link-pure.sc-phn-myporsche .login-status.sc-phn-myporsche,.myporsche.sc-phn-myporsche-h phn-p-button-pure.sc-phn-myporsche .login-status.sc-phn-myporsche{margin-top:unset;position:absolute;top:0}}@media (min-width: 1300px) and (max-width: 1759px){.myporsche.sc-phn-myporsche-h phn-p-link-pure.sc-phn-myporsche .login-status.sc-phn-myporsche,.myporsche.sc-phn-myporsche-h phn-p-button-pure.sc-phn-myporsche .login-status.sc-phn-myporsche{left:0;padding-left:1.75rem;padding-top:1.1875rem}}@media (min-width: 1760px){.myporsche.sc-phn-myporsche-h phn-p-link-pure.sc-phn-myporsche .login-status.sc-phn-myporsche,.myporsche.sc-phn-myporsche-h phn-p-button-pure.sc-phn-myporsche .login-status.sc-phn-myporsche{left:0;padding-left:2.1875rem;font-size:0.875rem;padding-top:1.6875rem}}.myporsche.sc-phn-myporsche-h .myporsche-logged-in.sc-phn-myporsche{padding:0.875rem 0 0.875rem 0;border-bottom:1px solid #e3e4e5}@media (min-width: 760px) and (max-width: 999px){.myporsche.sc-phn-myporsche-h .myporsche-logged-in.sc-phn-myporsche{padding:1.125rem 0 1.125rem 0}}@media (min-width: 1300px){.myporsche.sc-phn-myporsche-h .myporsche-logged-in.sc-phn-myporsche{display:none}}@media (min-width: 1300px){.myporsche.sc-phn-myporsche-h .myporsche-skeleton.sc-phn-myporsche{width:8rem;margin-top:0.125rem;width:6.375rem;height:0.875rem;background-color:#e3e4e5}}";

const Myporsche = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.myporscheLink = '';
        this.myporscheText = '';
        ...
        this.renderMyPorsche = (loggedInState) => {
            ...
            return this.isDesktop ? this.renderLoggedInFlyout() : this.renderLoggedInDrawer();
        };
    }
    resizeHandler() {
        this.isDesktop = isDesktop();
    ...
    renderLoggedOutComponent() {
        return (_commonjsHelpers.h("phn-p-link-pure", { style: navStateStore.get('navigationLoaded') ? {} : this.hiddenStyle, class: "myporsche-logged-out", href: this.myporscheLink, onClick: () => {
                const linkItem = {
                ...
                this.sendMetaFunctionClickAnalyticsEvent(linkItem);
            }, size: "inherit", icon: "user" }, this.myporscheText, _commonjsHelpers.h("phn-p-text", { class: "login-status", color: "inherit", size: "inherit", ellipsis: "true", align: "left" }, this.getSubTextForUser())));
    }
    renderLoadingComponent() {
        return (_commonjsHelpers.h("phn-p-button-pure", { class: "myporsche-loading", loading: "true", size: "inherit", "aria-label": "myporsche loading", style: navStateStore.get('navigationLoaded') ? {} : this.hiddenStyle }, this.myporscheText));
    }
    renderLoggedInFlyout() {
        const unreadMessagesCount = navContentStore.get('unreadMessagesCount');
        return (_commonjsHelpers.h("phn-myporsche-flyout", { style: navStateStore.get('navigationLoaded') ? {} : this.hiddenStyle, class: "myporsche-flyout", myPorscheText: this.myporscheText, loginStatus: this.getSubTextForUser(), unreadMessagesCount: unreadMessagesCount }));
    }
    renderLoggedInDrawer() {
        const unreadMessagesCount = navContentStore.get('unreadMessagesCount');
        return (_commonjsHelpers.h("phn-myporsche-drawer", { class: "myporsche-logged-in", myPorscheText: this.myporscheText, loginStatus: this.getSubTextForUser(), unreadMessagesCount: unreadMessagesCount }));
    }
    componentWillLoad() {
        this.isDesktop = isDesktop();
    }
    render() {
        const loggedInState = navContentStore.get('loggedInState');
        const hiddenStyle = { display: 'none' };
        return (_commonjsHelpers.h(_commonjsHelpers.Host, { class: "myporsche" }, this.renderMyPorsche(loggedInState), _commonjsHelpers.h("div", { class: "myporsche-skeleton", style: navStateStore.get('navigationLoaded') ? hiddenStyle : {} })));
    }
    get el() { return _commonjsHelpers.getElement(this); }
};
Myporsche.style = myporscheCss;

const myporscheDrawerCss = ".sc-phn-myporsche-drawer-h{display:block}.sc-phn-myporsche-drawer-h .login-status.sc-phn-myporsche-drawer{margin-top:-0.25rem;font-size:0.75rem}@media (min-width: 760px) and (max-width: 999px){.sc-phn-myporsche-drawer-h .login-status.sc-phn-myporsche-drawer{margin-top:-0.1875rem}}@media (min-width: 1300px){.sc-phn-myporsche-drawer-h .login-status.sc-phn-myporsche-drawer{margin-top:unset}}.sc-phn-myporsche-drawer-h .myporsche-drawer-trigger.sc-phn-myporsche-drawer:focus-within{outline-width:2px;outline-style:solid;outline-offset:1px;outline-color:currentColor}.sc-phn-myporsche-drawer-h .myporsche-drawer-trigger-label.sc-phn-myporsche-drawer{display:-ms-inline-flexbox;display:inline-flex;-ms-flex-direction:column;flex-direction:column;padding-left:0.25rem}.sc-phn-myporsche-drawer-h .myporsche-unread-messages-indicator.sc-phn-myporsche-drawer{position:absolute;width:0.375rem;height:0.375rem;border-radius:50%;background-color:#d5001c;left:1.0625rem;top:0.0625rem}.sc-phn-myporsche-drawer-h .myporsche-unread-messages-indicator.hidden.sc-phn-myporsche-drawer{display:none}";
...
const MyPorscheDrawer = class {
    constructor(hostRef) {
        ...
        this.openDrawer = () => {
            const phnMyporscheItemsDrawer = {
                content: _commonjsHelpers.h("phn-myporsche-items-drawer", { slot: "content", myPorscheText: this.myPorscheText })
            };
            navStateStore.set('drawerContentStack', [...navStateStore.get('drawerContentStack'), phnMyporscheItemsDrawer]);
        };
    }
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("phn-button-extended", { class: "myporsche-drawer-trigger", icon: "arrow-head-right", onClick: this.openDrawer, cancelTabIndex: false }, _commonjsHelpers.h("span", { class: this.unreadMessagesCount > 0
                ? 'myporsche-unread-messages-indicator'
                : 'myporsche-unread-messages-indicator hidden' }), _commonjsHelpers.h("phn-p-icon", { color: "inherit", name: "user", size: "small" }), _commonjsHelpers.h("span", { class: "myporsche-drawer-trigger-label" }, this.myPorscheText, _commonjsHelpers.h("phn-p-text", { class: "login-status", color: "inherit", size: "inherit", ellipsis: "true", align: "left" }, this.loginStatus)))));
    }
};
MyPorscheDrawer.style = myporscheDrawerCss;

const myporscheFlyoutCss = ".sc-phn-myporsche-flyout-h{display:none;position:relative}@media (min-width: 1300px){.sc-phn-myporsche-flyout-h{display:block}}.sc-phn-myporsche-flyout-h .flyout-trigger.sc-phn-myporsche-flyout{cursor:pointer}.sc-phn-myporsche-flyout-h .flyout-trigger.sc-phn-myporsche-flyout:focus{outline-width:2px;outline-style:solid;outline-offset:1px;outline-color:currentColor}@media (min-width: 1300px) and (max-width: 1759px){.sc-phn-myporsche-flyout-h .flyout-trigger.sc-phn-myporsche-flyout:first-child{padding-right:1.125rem}}@media (min-width: 1760px){.sc-phn-myporsche-flyout-h .flyout-trigger.sc-phn-myporsche-flyout:first-child{padding-right:1.5rem}}@media (min-width: 1300px){.sc-phn-myporsche-flyout-h .flyout-trigger.sc-phn-myporsche-flyout .login-status.sc-phn-myporsche-flyout{position:absolute;top:0;padding-bottom:0.625rem;left:0;padding-left:1.75rem;font-size:0.75rem;padding-top:1.1875rem}}@media (min-width: 1760px){.sc-phn-myporsche-flyout-h .flyout-trigger.sc-phn-myporsche-flyout .login-status.sc-phn-myporsche-flyout{left:0;padding-left:2.1875rem;font-size:0.875rem;padding-top:1.6875rem}}.sc-phn-myporsche-flyout-h .myporsche-items.sc-phn-myporsche-flyout{-webkit-box-sizing:border-box;box-sizing:border-box;display:block;position:absolute;background:#fff;right:-3rem;z-index:300}.sc-phn-myporsche-flyout-h .myporsche-items.sc-phn-myporsche-flyout phn-myporsche-items.sc-phn-myporsche-flyout{-webkit-box-sizing:inherit;box-sizing:inherit;border:1px solid #e3e4e5;position:relative;background:#fff}.sc-phn-myporsche-flyout-h .myporsche-items.sc-phn-myporsche-flyout phn-myporsche-items.sc-phn-myporsche-flyout:before{content:\"\";width:0;height:0;position:absolute;top:-0.375rem;right:50%;-webkit-transform:translateX(50%);transform:translateX(50%);border-style:solid;border-width:0 0.3125rem 0.3125rem;border-color:transparent transparent #e3e4e5}.sc-phn-myporsche-flyout-h .myporsche-items.sc-phn-myporsche-flyout phn-myporsche-items.sc-phn-myporsche-flyout:after{content:\"\";width:0;height:0;position:absolute;top:-0.3125rem;right:50%;-webkit-transform:translateX(50%);transform:translateX(50%);border-style:solid;border-width:0 0.3125rem 0.3125rem;border-color:transparent transparent #fff}@media (min-width: 1300px) and (max-width: 1759px){.sc-phn-myporsche-flyout-h .myporsche-items.sc-phn-myporsche-flyout{width:14.625rem;top:2.75rem}}@media (min-width: 1760px){.sc-phn-myporsche-flyout-h .myporsche-items.sc-phn-myporsche-flyout{width:17.125rem;top:3.5625rem}}@media (min-width: 1300px){.sc-phn-myporsche-flyout-h .myporsche-items.sc-phn-myporsche-flyout phn-myporsche-items.sc-phn-myporsche-flyout{padding:0.625rem 1.5rem 2.25rem 1.5rem}}.sc-phn-myporsche-flyout-h .myporsche-unread-messages-indicator.sc-phn-myporsche-flyout{position:absolute;width:0.375rem;height:0.375rem;border-radius:50%;background-color:#d5001c;left:1.0625rem;top:0.0625rem}@media (min-width: 1760px){.sc-phn-myporsche-flyout-h .myporsche-unread-messages-indicator.sc-phn-myporsche-flyout{width:0.5rem;height:0.5rem;left:1.25rem;top:0.0625rem}}.sc-phn-myporsche-flyout-h .myporsche-unread-messages-indicator.hidden.sc-phn-myporsche-flyout{display:none}";

const MyPorscheFlyout = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        this.env = DEFAULT_ENVIRONMENT;
        this.unreadMessagesCount = 0;
    ...
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("span", { "aria-hidden": "true", class: this.unreadMessagesCount > 0
                ? 'myporsche-unread-messages-indicator'
                : 'myporsche-unread-messages-indicator hidden' }), _commonjsHelpers.h("phn-p-button-pure", { class: "flyout-trigger", size: "inherit", icon: "user", active: this.isActive, onKeyDown: this.keyDownHandler, "aria-haspopup": "true", "aria-expanded": this.isActive ? 'true' : 'false', onClick: this.handleClick, id: this.screenReaderId, "aria-label": "open myporsche flyout" }, this.myPorscheText, _commonjsHelpers.h("phn-p-text", { class: "login-status", color: "inherit", size: "inherit", ellipsis: "true", align: "left" }, this.loginStatus)), this.isActive ? (_commonjsHelpers.h("div", { class: "myporsche-items", "aria-labelledby": this.screenReaderId, role: "region" }, _commonjsHelpers.h("phn-myporsche-items", { myPorscheCloseHandler: () => this.myPorscheCloseHandler() }))) : null));
    }
};
MyPorscheFlyout.style = myporscheFlyoutCss;

const myporscheItemsCss = ".sc-phn-myporsche-items-h{display:block}.sc-phn-myporsche-items-h ul.sc-phn-myporsche-items{margin:0;padding:0;text-indent:0;list-style-type:none}.sc-phn-myporsche-items-h li.sc-phn-myporsche-items{margin:0;padding:0;text-indent:0;list-style-type:none}.sc-phn-myporsche-items-h hr.sc-phn-myporsche-items{display:none}@media (min-width: 1300px){.sc-phn-myporsche-items-h hr.sc-phn-myporsche-items{display:block;height:1px;border:none;background-color:#e3e4e5;margin:0.875rem 0 0 0;text-align:center}}";

const MyPorscheItems = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
        ...
        this.displayListItems = (item, index) => {
            if (item.PhraseId === `${MYPORSCHE_PHRASE_ID}.messages`) {
                if (index === 0) {
                    return (_commonjsHelpers.h("li", null, _commonjsHelpers.h("phn-messages-link", { class: "myporsche-list-item", item: item, onKeyDown: this.handlePressShiftTabKey })));
                }
                return (_commonjsHelpers.h("li", null, _commonjsHelpers.h("phn-messages-link", { class: "myporsche-list-item", item: item })));
            }
            if (index === 0) {
                return (_commonjsHelpers.h("li", null, _commonjsHelpers.h("phn-list-item", { class: "myporsche-list-item", item: item, onKeyDown: this.handlePressShiftTabKey })));
            }
            return (_commonjsHelpers.h("li", null, _commonjsHelpers.h("phn-list-item", { class: "myporsche-list-item", item: item })));
        };
    }
    ...
    render() {
        if (navContentStore.get('metafunctionalities').length > 0) {
            ...
            return (_commonjsHelpers.h("ul", null, nonStaticItems.map(this.displayListItems), _commonjsHelpers.h("hr", { "aria-hidden": "true" }), _commonjsHelpers.h("li", null, _commonjsHelpers.h("phn-list-item", { class: "myporsche-list-item logout", item: this.getMyPorscheItemFromTemplateAsLinkItem('logout'), onKeyDown: this.handlePressTabKey }))));
        }
        return null;
    }
};
...
const myporscheItemsDrawerCss = ".sc-phn-myporsche-items-drawer-h{padding-bottom:3.125rem}";

const MyPorscheItemsDrawer = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    }
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("div", { class: "drawer-container" }, _commonjsHelpers.h("phn-headline", { text: this.myPorscheText, class: "drawer-headline" }), _commonjsHelpers.h("phn-myporsche-items", null))));
    }
};
MyPorscheItemsDrawer.style = myporscheItemsDrawerCss;

const previewBadgeCss = ".sc-phn-preview-badge-h{display:block;position:absolute;top:0;right:0;background-color:#d5001c;z-index:9999;padding:0 0.125rem}";

const PreviewBadge = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    }
    render() {
        return (_commonjsHelpers.h(_commonjsHelpers.Host, null, _commonjsHelpers.h("phn-p-text", { theme: "dark", size: "x-small" }, this.versionString)));
    }
};
PreviewBadge.style = previewBadgeCss;

const shopsAndMoreCss = ".shops-and-more.sc-phn-shops-and-more{display:-ms-flexbox;display:flex;-ms-flex-direction:row;flex-direction:row}@media (min-width: 1300px) and (max-width: 1759px){.shops-and-more.sc-phn-shops-and-more{padding-right:1.5rem}}@media (min-width: 1300px) and (max-width: 1759px){.shops-and-more.sc-phn-shops-and-more .shops-and-more-overlay-trigger.sc-phn-shops-and-more{padding:0 0.5625rem;height:91px}}@media (min-width: 1760px){.shops-and-more.sc-phn-shops-and-more .shops-and-more-overlay-trigger.sc-phn-shops-and-more{padding:0 0.75rem;height:102px}}";

const ShopsAndMore = class {
    constructor(hostRef) {
        _commonjsHelpers.registerInstance(this, hostRef);
    }
    getItems() {
    ...
    render() {
        const items = this.getItems();
        return (_commonjsHelpers.h("div", { class: "shops-and-more" }, items.map((item) => {
            return (_commonjsHelpers.h("phn-mega-fly-out-overlay", { class: "shops-and-more-overlay-trigger overlay-trigger", showOverlayHandler: this.showOverlayHandler, hideOverlayHandler: this.hideOverlayHandler, keyboardShowOverlayHandler: this.keyboardShowOverlayHandler, keyboardHideOverlayHandler: this.keyboardHideOverlayHandler }, _commonjsHelpers.h("phn-desktop-shop-and-more-item", { slot: "button", itemText: item.Text, class: "desktop-shop-and-more-item" }), _commonjsHelpers.h("phn-shops-and-more-overview", { slot: "content", item: item })));
        })));
    }
};
ShopsAndMore.style = shopsAndMoreCss;

const hasLinkItemChild = (item) => {
    if (item.Items) {
...
class NavigationLoadEvent extends AnalyticsEvent {
    constructor(analyticsSetup, item) {
        super(analyticsSetup);
        this.context.eventAction = 'PAGHomeNav_Navigation_Load';
        this.initComponentClick();
        this.populateComponentClickFromContentItem(item);
    }
    initComponentClick() {
        this.componentClick = {
            [ComponentClickCollectionDimension.ClickElementId]: undefined,
    ...
    populateComponentClickFromContentItem(item) {
...
const ShopsAndMoreOverview = class {
    ...
    renderLinkGroup(_items, _title) {
        return (_commonjsHelpers.h("div", { class: "link-group" }, _commonjsHelpers.h("phn-headline", { role: "heading", "aria-level": "2", text: _title, class: "head-line" }), _commonjsHelpers.h("ul", { class: "list-item-group" }, _items.map((linkItem) => {
            return (_commonjsHelpers.h("li", null, _commonjsHelpers.h("phn-list-item", { item: linkItem })));
        }))));
    }
    ...
    render() {
        return (_commonjsHelpers.h("div", { class: "shops-and-more-overview" }, _commonjsHelpers.h("div", { class: "shops-and-more-overview-content" }, hasLinkItemChild(this.item)
...
exports.phn_button_extended = ButtonExtended;
exports.phn_car_frontshot = CarFrontshot;
exports.phn_car_further_links = CarFurtherLinks;
exports.phn_car_further_links_v2 = CarFurtherLinksV2;
exports.phn_car_meta_functionalities = CarMetaFunctionalities;
exports.phn_car_meta_functionalities_v2 = CarMetaFunctionalitiesV2;
exports.phn_car_model = CarModel;
exports.phn_car_model_list = CarModelList;
exports.phn_car_model_list_v2 = CarModelListV2;
exports.phn_car_series = CarSeries;
exports.phn_car_series_list = CarSeriesList;
exports.phn_car_series_model_overview = CarSeriesModelOverview;
exports.phn_car_series_model_overview_v2 = CarSeriesModelOverviewV2;
exports.phn_crest = Crest;
exports.phn_dealer_search = DealerSearch;
exports.phn_desktop_shop_and_more_item = DesktopShopAndMoreItem;
exports.phn_drawer_car_series_model_overview = DrawerCarSeriesModelOverview;
exports.phn_drawer_shops_and_more = DrawerShopsAndMore;
exports.phn_header = Header;
exports.phn_headline = Headline;
exports.phn_list_item = ListItem;
exports.phn_mega_fly_out_overlay = MegaFlyOutOverlay;
exports.phn_menu_desktop_level_1 = MenuDesktopLevel1;
exports.phn_menu_drawer = MenuDrawer;
exports.phn_menu_mobile_level_1 = MenuMobileLevel1;
exports.phn_messages_link = MessagesLink;
exports.phn_meta_functionalities = MetaFunctionalities;
exports.phn_mobile_car_series_list = MobileCarSeriesList;
exports.phn_mobile_layout = MobileLayout;
exports.phn_mobile_shop_and_more_item = MobileShopAndMoreItem;
exports.phn_mobile_shops_and_more = MobileShopsAndMore;
exports.phn_myporsche = Myporsche;
exports.phn_myporsche_drawer = MyPorscheDrawer;
exports.phn_myporsche_flyout = MyPorscheFlyout;
exports.phn_myporsche_items = MyPorscheItems;
exports.phn_myporsche_items_drawer = MyPorscheItemsDrawer;
exports.phn_preview_badge = PreviewBadge;
exports.phn_shops_and_more = ShopsAndMore;
exports.phn_shops_and_more_overview = ShopsAndMoreOverview;
