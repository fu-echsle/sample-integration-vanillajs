...
const TextareaWrapper = class {
    constructor(hostRef) {
        ...
        this.labelId = index$1.randomString();
    }
    componentWillLoad() {
        this.setTextarea();
        this.setState();
    ...
    render() {
        const labelClasses = index$1.cx(index$1.prefix('textarea-wrapper__label'));
        const labelTextClasses = index$1.cx(index$1.prefix('textarea-wrapper__label-text'), index$1.mapBreakpointPropToPrefixedClasses('textarea-wrapper__label-text-', this.hideLabel, ['hidden', 'visible']), this.disabled && index$1.prefix('textarea-wrapper__label-text--disabled'));
        const fakeTextareaClasses = index$1.cx(index$1.prefix('textarea-wrapper__fake-textarea'), index$1.prefix(`textarea-wrapper__fake-textarea--${this.state}`), this.disabled && index$1.prefix('textarea-wrapper__fake-textarea--disabled'), this.readonly && index$1.prefix('textarea-wrapper__fake-textarea--readonly'));
        const messageClasses = index$1.cx(index$1.prefix('textarea-wrapper__message'), index$1.prefix(`textarea-wrapper__message--${this.state}`));
        return (index.h(index.Host, null, index.h("label", { class: labelClasses, id: this.state === 'error' && this.labelId }, this.isLabelVisible &&
            index.h("p-text", { class: labelTextClasses, color: 'inherit', tag: 'span', onClick: () => this.labelClick() }, this.label ? this.label : index.h("span", null, index.h("slot", { name: 'label' }))), index.h("span", { class: fakeTextareaClasses }, index.h("slot", null))), this.isMessageVisible &&
            index.h("p-text", { class: messageClasses, color: 'inherit', role: this.state === 'error' && 'alert', "aria-describedby": this.state === 'error' && this.labelId }, this.message ? this.message : index.h("span", null, index.h("slot", { name: 'message' })))));
    }
    get isLabelVisible() {
    ...
    bindStateListener() {
        index$1.transitionListener(this.textarea, 'border-top-color', () => {
            this.setState();
        });
    }
    addSlottedStyles() {
