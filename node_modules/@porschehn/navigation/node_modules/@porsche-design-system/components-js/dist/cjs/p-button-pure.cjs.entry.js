...
const buttonHandling = require('./buttonHandling-2de95500.js');
...
const ButtonPure = class {
    ...
    componentDidLoad() {
        ...
        index$1.transitionListener(this.buttonTag, 'font-size', () => {
            const size = index$1.calcLineHeightForElement(this.buttonTag);
            this.iconTag.style.width = `${size}em`;
            this.iconTag.style.height = `${size}em`;
        });
    }
    render() {
        const buttonPureClasses = index$1.cx(index$1.prefix('button-pure'), index$1.mapBreakpointPropToPrefixedClasses('button-pure--size', this.size), index$1.prefix(`button-pure--theme-${this.theme}`));
        const iconClasses = index$1.cx(index$1.prefix('button-pure__icon'));
        const labelClasses = index$1.cx(index$1.prefix('button-pure__label'), index$1.mapBreakpointPropToPrefixedClasses('button-pure__label-', this.hideLabel, ['hidden', 'visible']));
        return (index.h("button", { class: buttonPureClasses, type: this.type, disabled: this.isDisabled(), tabindex: this.tabbable ? 0 : -1, ref: el => this.buttonTag = el, "aria-busy": this.loading && 'true' }, this.loading ? (index.h("p-spinner", { class: iconClasses, size: 'inherit', theme: this.theme, ref: el => this.iconTag = el })) : (index.h("p-icon", { class: iconClasses, color: 'inherit', size: 'inherit', name: this.icon, source: this.iconSource, ref: el => this.iconTag = el, "aria-hidden": 'true' })), index.h("p-text", { class: labelClasses, tag: 'span', color: 'inherit', size: 'inherit', weight: this.weight }, index.h("slot", null))));
    }
    isDisabled() {
