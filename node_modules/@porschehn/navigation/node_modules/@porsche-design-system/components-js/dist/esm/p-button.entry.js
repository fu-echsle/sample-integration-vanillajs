...
const Button = class {
    ...
    render() {
        const buttonClasses = cx(prefix('button'), prefix(`button--${this.variant}`), mapBreakpointPropToPrefixedClasses('button-', this.hideLabel, ['without-label', 'with-label']), prefix(`button--theme-${this.theme}`));
        const iconClasses = prefix('button__icon');
        const labelClasses = prefix('button__label');
        return (h("button", { class: buttonClasses, type: this.type, disabled: this.isDisabled(), tabindex: this.tabbable ? 0 : -1, "aria-busy": this.loading && 'true' }, this.loading ? (h("p-spinner", { class: iconClasses, size: 'inherit', theme: this.variant === 'tertiary' && this.theme === 'light' ? 'light' : 'dark' })) : (h("p-icon", { class: iconClasses, size: 'inherit', name: this.icon, source: this.iconSource, color: 'inherit', "aria-hidden": 'true' })), h("p-text", { class: labelClasses, tag: 'span', color: 'inherit' }, h("slot", null))));
    }
    isDisabled() {
